...
**Loopback serve daemon (`--http`).** For clients that make many calls or run continuously (e.g. CrewAI crews), run `aimeat connect serve --http` instead of the default stdio mode: a long-lived loopback daemon on `127.0.0.1` that holds **one persistent WebSocket per agent** to the node (forward API calls + realtime task delivery, no polling) and exposes a local Streamable-HTTP MCP endpoint (`/v1/mcp`), a REST proxy (`/v1/*`), and a push long-poll (`/local/tasks/next`), advertised via the discovery file `<AIMEAT_HOME>/serve.json`. It degrades transparently to direct HTTP + polling when the node has the tunnel disabled. The default stdio mode stays for one-shot and CI/serverless use.
