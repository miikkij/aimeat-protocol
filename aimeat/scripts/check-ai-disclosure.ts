...
const AI_PROVENANCE_REVIEWED_WITHOUT = [
  ...
  // covers text "published with the purpose of informing the public on matters of public interest",
  // and exempts even that where the content has had "human review or editorial control" with
  // somebody holding editorial responsibility. A message to one customer is neither published to the
  // public nor about a matter of public interest, and the person pressing send IS the editorial
  // control. Paragraph 2 does cover text and does want a machine-readable mark, but it is the
  // obligation of whoever PROVIDES the generating system about that system's outputs, and this node
  // already mints a provenance record for every completion it runs.
  //
  // SO IT IS OPTIONAL AND IT IS A HEADER, not a provenance stamp and not a line in the body.
  // `ai_disclosure` on the send takes the IETF AI-Disclosure draft's four words and puts them on the
  // wire as X-AI-Disclosure, on all three send paths — the caller's own mailbox, a company's SMTP
  // and the node's shared sender — because a mark that appeared on only one of them would make the
  // others look human. The audience is machines: nobody reading their inbox follows a link to a
  // hash, which is why a provenance ID was the wrong answer to this question.
  //
  // A declaration cannot then be suppressed: there is no parameter for leaving the header off. What
