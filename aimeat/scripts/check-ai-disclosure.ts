...
const AI_PROVENANCE_REVIEWED_WITHOUT = [
  ...
  'aimeat_app_seo_set',
  // REVIEWED 2026-08-26, AND THE QUESTION IS OPEN — listed here so the gate does not decide it by
  // blocking, not because it is settled. Raised for the developer.
  //
  // Why it is not stamped TODAY. A provenance record describes stored content and is addressable at
  // /v1/provenance/{id}; a sent message is not stored here. The send log keeps a 300-character
  // subject and no body, so there would be nothing for the record to describe and nothing a reader
  // could ever follow the link back to. And the same message goes out through POST
  // /v1/outbound/send, which takes the same free text with no declaration: stamping one door and
  // not the other is precisely the MCP/REST drift the August 2026 audit measured, and would teach
  // callers that the declaration is arbitrary.
  //
  // Why the question is nevertheless real, and bigger than this tool. An AI-written message
  // arriving in a customer's inbox is content generated by AI reaching a person, which is the
  // strongest disclosure case anywhere in this codebase — stronger than a workspace record only
  // members read. But the answer is not a provenance ID: nobody reading their mail follows a link
  // to a hash. It is a line in the MESSAGE, and whether a message this node sends in its owner's
  // name says an AI wrote it is a decision about what the product tells a customer, not a
  // validator's to make. It belongs with the marketing-email work, where the unsubscribe line and
  // the sender identity are already being decided.
  'aimeat_mail_send',
