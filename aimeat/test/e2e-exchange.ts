...
await test('A usage note past the ODPS restrictions cap is refused on write, and an already-stored one lists whole with a warning', async () => {
  ...
  const listing = changes.find(c => (c.action === 'unchanged' || c.action === 'created') && c.label === `${LONG_APP}/brief`);
  assert(!!listing?.offeringId, `a warning never stops the listing: ${JSON.stringify(changes)}`);
  const row = changes.find(c => c.action === 'warning' && c.reason === 'ODPS_FIELD_TOO_LONG product.license.scope.restrictions');
  // The node's own sentences for these flags are 98 characters, then a space, then the note.
  assert(!!row && row.label === `${LONG_APP}/brief` && row.odpsField?.maxLength === 255 && row.odpsField?.length === 99 + note.length,
    `the overrun is reported with its numbers: ${JSON.stringify(row)}`);
  assert(String(row.message).includes('usageTerms.note') && String(row.message).includes('156'),
    `the message says how much of the cap the note has: ${row.message}`);

  // A real validator agrees with the warning, and on that field only: the rest of the document is in schema.
  const doc = parseYaml(await (await fetch(`${BASE}/v1/exchange/offerings/${listing.offeringId}/odps.yaml`)).text()) as any;
  const validatable = { ...doc, product: { ...doc.product } };
  delete validatable.product.dataAccess;           // the published schema's known contradiction, see above
  delete validatable.product.paymentGateways;
  const validator = new Ajv2020({ strict: false, allErrors: true });
  addFormats(validator);
  validator.validate(odpsSchema, validatable);
  const tooLong = (validator.errors ?? []).filter(e => e.keyword === 'maxLength').map(e => e.instancePath);
  assert(JSON.stringify(tooLong) === JSON.stringify(['/product/license/scope/restrictions']), `validator refuses exactly the warned field: ${JSON.stringify(tooLong)}`);
  assert(String(doc.product.license.scope.restrictions).endsWith(note), 'and the restriction is published as written, never cut');
});
