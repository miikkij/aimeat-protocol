 ...
 *   v1.5.0 — 2026-05-28 — Verify owner-scoped generator artifacts through the project API instead of missing-key memory reads
 */
...
await test('Agent: save interview spec (must be readable back with agent token)', async () => {
    ...
    // Generator artifacts are owner-scoped; agents read them through the generator project endpoint.
    const { status: readStatus, body: readBody } = await json(`/v1/generator/${generatorApiProjectId}`, {
        headers: { Authorization: `Bearer ${generatorAgentToken}` },
    });
    assert(readStatus === 200, `Agent should read generator project, got ${readStatus}`);
...
\`\`\``;
    ...
    const { status: memStatus, body: memBody } = await json(`/v1/generator/${generatorApiProjectId}`, {
        headers: { Authorization: `Bearer ${generatorAgentToken}` },
    });
