/**
 * @file test/unit/decide-systemone-client.test.ts
 * @author Jouni Miikki
 * SPDX-License-Identifier: MIT
 * @description The System One client against a fake fetch and a fake clock: success, Retry-After
 *   honoured, 5xx retried then given up, 422 not retried, per-attempt timeout retried, a malformed
 *   answer refused, the key absent from every error, no authorization header without a key, and an
 *   adapter that reshapes the answer and nothing else.
 * @version-history
 *   v1.1.0 — 2026-09-23 — The generic client (was decide-jev-client.test.ts): a keyless call and
 *     the Laya adapter.
 *   v1.0.0 — 2026-09-19 — Initial.
 */
import { describe, expect, it } from 'vitest';
import { callSystemOne, SystemOneError, type SystemOneFetch, type SystemOneRequest } from '../../src/services/decide/systemone-client.js';
...
function script(steps: Array<Response | 'hang' | Error>) {
  const calls: Array<{ url: string; init: Parameters<SystemOneFetch>[1] }> = [];
  const fetchImpl: SystemOneFetch = (url, init) => {
    calls.push({ url, init });
...
async function failure(p: Promise<unknown>): Promise<SystemOneError> {
  try { await p; } catch (e) { if (e instanceof SystemOneError) return e; throw e; }
  throw new Error('expected a SystemOneError');
}
function noKey(e: SystemOneError) {
...
describe('callSystemOne', () => {
  it('returns the validated answers and sends the call the SDKs send', async () => {
    ...
    const c = fakeClock();
    const res = await callSystemOne({ url: ENDPOINT, key: KEY, request, fetchImpl, ...c });
  ...
  it('takes the request id from the header when the body has none', async () => {
    ...
    const res = await callSystemOne({ url: ENDPOINT, key: KEY, request, fetchImpl, ...fakeClock() });
  ...
  it('honours Retry-After in seconds', async () => {
    ...
    const c = fakeClock();
    await callSystemOne({ url: ENDPOINT, key: KEY, request, fetchImpl, ...c });
  ...
  it('gives up on 503 after three attempts with JEV_SERVER, backing off 500 then 1000 less jitter', async () => {
    ...
    const c = fakeClock();
    const e = await failure(callSystemOne({ url: ENDPOINT, key: KEY, request, fetchImpl, random: () => 0.5, ...c }));
  ...
  it('does not retry a 422 and carries the provider detail', async () => {
    ...
    const c = fakeClock();
    const e = await failure(callSystemOne({ url: ENDPOINT, key: KEY, request, fetchImpl, ...c }));
  ...
  it('does not retry 400, 401, 403 or 404', async () => {
    for (const [status, code] of [[400, 'JEV_BAD_REQUEST'], [401, 'JEV_UNAUTHORIZED'], [403, 'JEV_FORBIDDEN'], [404, 'JEV_NOT_FOUND']] as const) {
      ...
      const e = await failure(callSystemOne({ url: ENDPOINT, key: KEY, request, fetchImpl, ...fakeClock() }));
      expect(calls).toHaveLength(1);
  ...
  it('retries a per-attempt timeout, then succeeds', async () => {
    ...
    const c = fakeClock();
    const res = await callSystemOne({ url: ENDPOINT, key: KEY, request, fetchImpl, policy: { attemptTimeoutMs: 20 }, random: () => 0, ...c });
    expect(calls).toHaveLength(2);
  ...
  it('reports JEV_TIMEOUT when every attempt times out', async () => {
    ...
    const e = await failure(callSystemOne({ url: ENDPOINT, key: KEY, request, fetchImpl, policy: { attemptTimeoutMs: 10, maxRetries: 1 }, ...fakeClock() }));
    expect(calls).toHaveLength(2);
  ...
  it('ignores a Retry-After longer than maxRetryAfterMs and backs off instead', async () => {
    ...
    const c = fakeClock();
    await callSystemOne({ url: ENDPOINT, key: KEY, request, fetchImpl, random: () => 0, ...c });
    expect(c.slept).toEqual([500]);
  });
  ...
  it('refuses a malformed answer without retrying', async () => {
    ...
    expect((await failure(callSystemOne({ url: ENDPOINT, key: KEY, request, fetchImpl, ...fakeClock() }))).code).toBe('JEV_BAD_RESPONSE');
  });
  ...
  it('never puts the key in an error, even when the provider echoes it', async () => {
    ...
    for (const status of [401, 422, 500, 200]) {
      ...
      const e = await failure(callSystemOne({ url: ENDPOINT, key: KEY, request, fetchImpl, policy: { maxRetries: 0 }, ...fakeClock() }));
      expect(e).toBeInstanceOf(SystemOneError);
      noKey(e);
    }
  });

  it('sends no authorization header when the provider takes no key, and names the provider in errors', async () => {
    const { calls, fetchImpl } = script([json(200, okBody)]);
    await callSystemOne({ url: 'http://127.0.0.1:8801/v1/systemone', key: null, request, fetchImpl, ...fakeClock() });
    const headers = calls[0].init.headers as Record<string, string>;
    expect(Object.keys(headers).map(h => h.toLowerCase())).not.toContain('authorization');
    const bad = script([json(500, {})]);
    const e = await failure(callSystemOne({ url: ENDPOINT, key: null, request, fetchImpl: bad.fetchImpl, providerName: 'Laya', policy: { maxRetries: 0 }, ...fakeClock() }));
    expect(e.message).toContain('Laya');
  });

  it('the laya adapter pins the checkpoint that answered, and an unknown adapter changes nothing', async () => {
    const laya = { ...okBody, model: 'laya-rl-agent', routing: { model: 'multilingual' } };
    const a = script([json(200, laya)]);
    expect((await callSystemOne({ url: ENDPOINT, key: null, request, fetchImpl: a.fetchImpl, adapter: 'laya', ...fakeClock() })).model).toBe('multilingual');
    const b = script([json(200, laya)]);
    expect((await callSystemOne({ url: ENDPOINT, key: null, request, fetchImpl: b.fetchImpl, ...fakeClock() })).model).toBe('laya-rl-agent');
    // The request is the same whichever adapter reads the answer.
    expect(a.calls[0].init.body).toBe(b.calls[0].init.body);
  });

  it('stops when the caller cancels', async () => {
    const ctl = new AbortController();
    const { calls, fetchImpl } = script(['hang']);
    const p = failure(callSystemOne({ url: ENDPOINT, key: KEY, request, fetchImpl, signal: ctl.signal, ...fakeClock() }));
    ctl.abort();
    const e = await p;
    expect(calls).toHaveLength(1);
