 ...
 * @usage cd aimeat && pnpm vitest run test/unit/app-owner-normalization.test.ts
 * @version-history
...
/** Assert with a descriptive message (vitest surfaces the 2nd arg on failure). */
...
describe('app owner hygiene migrations (SqliteStorage)', () => {
  it('normalizeAppOwnerNames rewrites GHII-form ownerName to the bare name', async () => {
  ...
  it('mergeForkedAppBuckets folds stray buckets into the canonical GHII bucket', async () => {
