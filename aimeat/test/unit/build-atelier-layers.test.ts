...
describe('the Atelier specification in parts', () => {
  ...
  it('the first part names the others, says which come before code, and carries the token', () => {
    ...
    for (const id of ['genre', 'patterns', 'look']) expect(start).toContain('`' + id + '`');
    expect(start).toMatch(/Read `genre` and `patterns` BEFORE you write any code/);
  ...
  it('answers null for an id it does not have', () => {
    expect(atelierPiece(full, 'libraries', config.baseUrl)).toBeNull();
  });
});
