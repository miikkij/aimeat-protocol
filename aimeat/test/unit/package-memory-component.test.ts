 ...
 *
 *   A package installs under the OWNER whoever presses install, so its memory component (and a
 *   translation component, which writes one `i18n.` record) writes into the owner's namespace. The
 *   memory door asks memory:write for any write, and memory:write-as-owner of an agent writing into
 *   the owner's namespace from outside it; an app grant's own namespace is the owner's. The owner in
 *   person passes, as at every door. The E2E half is e2e-package-components Part H.
 * @usage pnpm exec vitest run test/unit/package-memory-component.test.ts
 * @version-history
 *   v1.0.0 — 2026-09-24 — Initial.
 */
import { describe, it, expect } from 'vitest';
import { memoryComponentWriteRefusal } from '../../src/services/package-memory-component.js';
...
describe('memoryComponentWriteRefusal', () => {
    ...
    it('a translation component writes into the owner\'s memory too', () => {
        const out = memoryComponentWriteRefusal([{ id: 'fi', type: 'translation' }], agent(['packages:write']), OWNER);
        expect(out?.code).toBe('SCOPE_DENIED');
    });
