 ...
 *   - Every OTHER process (the runner, a suite, a fake provider) stops collecting at once: the
 *     measure is what the running node executed, not what a test imported. Node still tries to
 *     write at exit and prints "Failed to get 'result' from coverage profile response" to stderr;
 *     that line is the cost of the choice, and it breaks nothing.
...
if (dir) {
  if (isNodeProcess) {
    ...
    if (snapshotMs > 0) setInterval(flush, snapshotMs).unref();
    setInterval(() => {
