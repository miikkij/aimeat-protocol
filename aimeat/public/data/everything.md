...
| <a id="g-1-attach-another-mcp-server"></a> **Attach another MCP server** | Connect your issue tracker, wiki or any other MCP server to your node once, and everything acting for you can use it: your own AI, your agents, your crews. The node holds the credential and spends it for you, so it goes in one place instead of into every client you own. A server that stops working says so, and says what would repair it. Attaching needs the `mcp:manage` permission, which is deliberately not part of full access. | `POST /v1/mcp-servers` · `aimeat_mcp_attach` |
| <a id="g-1-use-what-you-attached-from-chat"></a> **Use what you attached, from chat** | Three tools however many servers you have attached: which ones you can use, what each one can do, and call one. The credential never reaches the caller, and no caller ever names an address. | `aimeat_mcp_list` · `aimeat_mcp_tools` · `aimeat_mcp_call` |
...
| <a id="g-1-a-server-s-own-tools-in-your-tool-list"></a> **A server's own tools in your tool list** | For a server you use enough to spend the space on, its tools appear in your AI's list under their real names and real schemas instead of behind one call tool. Opt in per server, so the list does not grow with everything you ever attached. | `aimeat_mcp_update` with exposure |
| <a id="g-1-the-node-s-own-mcp-registry"></a> **The node's own MCP registry** | The operator attaches a server once and offers it to everybody with an account here, or to named owners only, with a price per call. A refusal is recorded, so the operator can see who wanted something they could not reach. Operator in person: an agent holding their credential cannot attach one. | `aimeat_mcp_registry_list` · `aimeat_mcp_registry_set` |
| <a id="g-1-a-server-that-belongs-to-a-team"></a> **A server that belongs to a team** | Attach the team wiki to the organism and the team reaches it. Nobody hands a token to each person who joins and nobody loses access when the person who set it up leaves. Bind it to one workspace instead and the workspace's own words decide: a contributor may call it, a viewer only sees it is there. | `POST /v1/mcp-servers/organism` |
...
| <a id="g-1-a-local-mcp-server-on-the-node"></a> **A local MCP server on the node** | Whoever runs the node can have it start an MCP server as a program on its own host, rather than calling one over the network. Off by default and behind two settings: the switch, and the exact commands that may run, compared whole. Only the operator in person can attach one; no owner door and no agent can name a command. | `POST /v1/mcp-servers/node` with `command` |
