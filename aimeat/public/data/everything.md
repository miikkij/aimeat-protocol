...
| <a id="g-10-build-spec-with-a-token"></a> **Build spec with a token** | The canonical build prompt hands out a token; the publish says whether the app was built against today's spec. | `/v1/prompts/build-app`, `/v1/prompts/build-app/core` (the part every app needs; the rest by section), `aimeat_handbook_get { tier: "build-app" }` over MCP, `/v1/prompts/build-app-atelier`, `/v1/how-an-app-builds` |
