...
| `katex@0/katex.min.js` + `katex.min.css` + `fonts/*.woff2` | `katex` (`window.katex`; maths typesetting, lazy-loaded by `aimeat-science`). The stylesheet is the published one with the woff and ttf entries removed from each `@font-face` src — the pre-2015 fallbacks in the same list, 876 kB nobody requests — so only the twenty woff2 faces are carried. The CSS must be linked beside the script; without it every formula renders as unspaced glyphs. | 0.18.5 | `katex` | `npm katex@0.18.5` (`dist/`) | MIT |
