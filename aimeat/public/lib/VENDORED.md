...
| `katex@0/katex.min.js` + `katex.min.css` + `fonts/*.woff2` | `katex` (`window.katex`; maths typesetting, lazy-loaded by `aimeat-living` when a formula is printed). The stylesheet is the published one with the woff and ttf entries removed from each `@font-face` src — the pre-2015 fallbacks in the same list, 876 kB nobody requests — so only the twenty woff2 faces are carried. The CSS must be linked beside the script; without it every formula renders as unspaced glyphs. | 0.18.5 | `katex` | `npm katex@0.18.5` (`dist/`) | MIT |
...
| `motion@13.min.js` | `motion` (UMD — `window.Motion`; the vanilla core under Framer Motion: `animate`, `scroll`, `inView`, `stagger`, `spring`, `press`, `hover`) | 13.1.1 | `motion` | `https://cdn.jsdelivr.net/npm/motion@13.1.1/dist/motion.js` | MIT |
| `anime@4.min.js` | `animejs` (UMD — `window.anime`, a NAMESPACE in v4: `anime.animate`, `anime.createTimeline`, `anime.stagger`, `anime.utils`) | 4.5.0 | `anime` | `https://cdn.jsdelivr.net/npm/animejs@4.5.0/dist/bundles/anime.umd.min.js` | MIT |
| `lenis@1.min.js` + `lenis@1.css` | `lenis` (IIFE — `globalThis.Lenis`; smooth scroll. The CSS is REQUIRED, not optional styling) | 1.3.26 | `lenis` | `https://cdn.jsdelivr.net/npm/lenis@1.3.26/dist/` | MIT |
