 ...
 * @description The front page as the message frame says it (design round, 2026-09-14, TARGET-075):
 *   say what you want, then make sure it happens, and AIMEAT is where it happens. The page keeps
 *   the showroom's face (ink, sun, coral, capital headlines, hard shadows, the 80s pictures) and
 *   changes the order and the words. Every sentence names an outcome, never a capability.
 *
 *   THIS IS A SEPARATE PAGE. It is routed at /v1/portal-v2 and reuses the showroom's blocks where
 *   the frame keeps them (the live counters, the store, the safety list); the front page at
 *   /v1/portal is untouched, so the two can be looked at side by side until one is chosen.
 *
 *   The order, top to bottom: the hero (the claim, the wish box, the two doors, the picture, the
 *   counters) · ten seconds under the hood (six outcomes, then the question, then the answer that
 *   is the hinge of the whole page) · the slide projector (optional, for whoever wants to see
 *   everything; landing-v2-projector.js) · prompts and what they produced (landing-v2-cards.js) ·
 *   the wall, the community first (same file) · the store · why the safety list is checkable ·
 *   the safety list · the last word.
 * @structure default export Landing2 · Hero2 · WishBox2 · TenSeconds · LinuxLine · Close2
 * @usage routed at /v1/portal-v2 by spa.html (and a spaRoute in routes/portal.ts, or F5 is a 404)
 * @version-history
 *   v0.1.0 — 2026-09-14 — Design round, built to "Viestikehys normipopulaatiolle" (Marketing,
 *     doc-mu0fimbx8fkn) and TARGET-075. Not the front page yet.
 */
 ...
 * a new person makes an account on the way and loses nothing.
 */
function WishBox2({ navigate }) {
  ...
  const go = (e) => {
    ...
    showLoginModal({ tab: 'register', onLogin: () => navigate('/v1/chat') });
  };
  return html`
...
export default function Landing2({ navigate }) {
  return html`
    <div class="ld ld-v2">
      <!-- 1. The hero: the claim, the wish box, the two doors, the picture, the live counters. -->
      ...
      <!-- 6. Liked the demo? Get your own. The showroom's block, unchanged. -->
