...
export function RegisterSection({ usecases, questionnaire, saving, onSave, onDraft, drafting }) {
  ...
  /**
   * Load the node's own draft into the editor, unsaved.
   *
   * Everything the node could work out arrives filled in; nothing is stored until Save. The button
   * replaced a list of one-click proposals per model, which on a real installation was fifteen rows
   * and a hundred and eighty dropdowns — an obstacle course made out of bookkeeping the node was
   * already holding.
   */
  const loadDraft = async () => {
    const proposed = await onDraft();
    if (!proposed) return;
    // Merged, not replaced: an operator who has already written entries keeps them, and the draft
    // adds what is missing. Matching by id, which the draft derives from the agent or app name.
    setDraft((prev) => {
      const base = prev ?? usecases;
      const have = new Set(base.map(u => u.id));
      return [...base, ...proposed.filter(u => !have.has(u.id))];
    });
    setOpenId(null);
  };
  ...
  return html`
    <section class="adm-cmp-section">
      ...
      <div class="adm-cmp-proposals adm-cmp-no-print">
        <p class="adm-cmp-note">${t('admin.compliance.draftNote')}</p>
        <button type="button" class="btn-primary" disabled=${drafting} onClick=${loadDraft}>
          ${drafting ? t('admin.compliance.drafting') : t('admin.compliance.draftAction')}
        </button>
      </div>
