 ...
 *   ?id=theme:sampler&theme=dark&preview=1 wears the draft theme Themes & Styles sends
 * @version-history
 *   v1.4.0 — 2026-09-24 — `preview=1`: the page wears the draft theme Themes & Styles sends.
...
function params() {
  ...
  return { id: q.get('id') || '', v: Number(q.get('v') || 0), theme: q.get('theme') === 'dark' ? 'dark' : 'light', solo: q.get('solo') === '1', preview: q.get('preview') === '1' };
}
...
/**
 * Wear the draft theme the editor sends: its stylesheet (generated by the node for the id `draft`)
 * as an adopted sheet, which no content policy blocks, and `data-palette="draft"` on the page. The
 * frame asks for the current draft as it opens, so a frame that loads late still shows it.
 */
function usePreviewTheme(on) {
  useEffect(() => {
    if (!on || typeof BroadcastChannel === 'undefined') return undefined;
    const channel = new BroadcastChannel(PREVIEW_CHANNEL);
    const sheet = new CSSStyleSheet();
    document.adoptedStyleSheets = [...document.adoptedStyleSheets, sheet];
    channel.onmessage = (ev) => {
      const css = ev?.data?.type === 'draft' ? ev.data.css : null;
      if (typeof css !== 'string') return;
      try { sheet.replaceSync(css); } catch (e) { swallowed('design-lab frame: draft theme', e); return; }
      document.documentElement.setAttribute('data-palette', 'draft');
    };
...
export default function DesignLabFrame() {
  const { id, v, theme, solo, preview } = params();
  const demo = useMemo(() => demoFor(id), [id]);
  usePreviewTheme(preview);
