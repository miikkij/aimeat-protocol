 ...
 * @structure DesignLabFrame (default) — reads ?id=&v=&theme=&solo=&preview=&look=&style=&fk= · soloLayout
...
import { measureChecks } from './frame-checks.js';
...
function params() {
  ...
  return {
    id: q.get('id') || '', v: Number(q.get('v') || 0), theme: q.get('theme') === 'dark' ? 'dark' : 'light', solo: q.get('solo') === '1',
    preview: q.get('preview') || '', look: q.get('look') || '', style: q.get('style') || '', fk: q.get('fk') || '',
  };
}

/** The channel Themes & Styles sends a draft theme's stylesheet on (same origin only). */
const PREVIEW_CHANNEL = 'aimeat-theme-preview';

/** The style on the page: the house style is no attribute, as everywhere. */
function wearStyle(style) {
  if (style && style !== 'aimeat') document.documentElement.setAttribute('data-palette', style);
  else document.documentElement.removeAttribute('data-palette');
}

/**
 * Wear the draft the editor sends under this frame's key (`preview=<key>`): its stylesheet as an
 * adopted sheet, which comes after every sheet of the page, and the style given. The frame asks for
 * the current draft as it opens, so a frame that loads late still shows it. `onSheet` runs after
 * every sheet it takes, for the checks.
 */
function usePreviewTheme(key, style, onSheet) {
  useEffect(() => {
    if (!key || typeof BroadcastChannel === 'undefined') return undefined;
    ...
    channel.onmessage = (ev) => {
      const d = ev?.data || {};
      if (d.type !== 'draft' || d.key !== key || typeof d.css !== 'string') return;
      try { sheet.replaceSync(d.css); } catch (e) { swallowed('design-lab frame: draft theme', e); return; }
      onSheet();
    };
    channel.postMessage({ type: 'ready', key });
    return () => { channel.close(); document.adoptedStyleSheets = document.adoptedStyleSheets.filter((s) => s !== sheet); };
  }, [key, style, onSheet]);
}

/**
 * Wear a saved theme and one of its styles (`look=<theme>&style=<style>`), offered or not: the lab's
 * theme and style picker. The shell's look script links the theme's sheet.
 */
function useSavedLook(look, style) {
  useEffect(() => {
    if (!look) return undefined;
    let alive = true;
    const L = /** @type {any} */ (window).__aimeatLook;
    apiGet(`/v1/themes/${encodeURIComponent(look)}`).then((r) => {
      const theme = r?.data?.theme;
      if (!alive || !theme || !L) return;
      const st = theme.styles.find((s) => s.id === style) || theme.styles.find((s) => s.id === theme.defaultStyle);
      L.show({ theme: theme.builtin ? null : theme.id, style: st?.id, sheet: theme.builtin ? null : `/v1/themes/${encodeURIComponent(theme.id)}/theme.css?v=${encodeURIComponent(theme.updatedAt || '')}` });
    }).catch((e) => swallowed('design-lab frame: saved theme', e));
    return () => { alive = false; };
  }, [look, style]);
}
...
export default function DesignLabFrame() {
  const { id, v, theme, solo, preview, look, style, fk } = params();
  ...
  // Themes & Styles asks what the operator's CSS does to the controls and words drawn here (`fk` is
  // the key it knows this frame by). Measured after the draft sheet lands and whenever the stage changes.
  const sendChecks = useCallback(() => {
    if (!fk || !stage.current || window.parent === window) return;
    requestAnimationFrame(() => {
      if (!stage.current) return;
      window.parent.postMessage({ type: 'theme-checks', fk, findings: measureChecks(stage.current) }, window.location.origin);
    });
  }, [fk]);
  usePreviewTheme(preview, style, sendChecks);
  useSavedLook(look, style);
