 ...
 * @version-history
 *   2026-09-13: Shared masthead and actions replace the home-only header layout.
...
import { Masthead, Stack, Action, Text } from '/components/poster-parts.js';
...
export function HomeHeader({ name, owner, identity, onOpenSettings }) {
  ...
  return html`
    <${Masthead} title=${name} size="large"
      mark=${html`<span aria-hidden="true" dangerouslySetInnerHTML=${{ __html: avatarSvg }}></span>`}
      identity=${identity && html`<${Text} kind="mono" tone="muted"
        title=${tr('home.identityHint', 'GHII: your permanent address. People, homes and agents everywhere find you by it.')}>${identity}<//>`}
      actions=${html`<${Stack} direction="wrap" align="center">
        <${Action} href="/v1/profile">${tr('home.settings.allControls', 'All settings and controls')} →<//>
        <${Action} onClick=${onOpenSettings}>${Cog} ${tr('home.settings.open', 'Home settings')}<//>
      <//>`} />`;
}
