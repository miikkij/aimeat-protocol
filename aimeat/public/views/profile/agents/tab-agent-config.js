...
export default function TabAgentConfig({ agentName, session, showToast }) {
  ...
  if (files.length === 0) {
    return html`<div class="agd-empty">${t('profile.agents.detail.empty.agentConfig')}</div>`;
  }
      ...
      ${selectedFile && html`
        <div>
          <div class="pf-agd-config-preview-header">
            ${t('profile.agents.detail.agentConfig.viewing')}: ${files.find(f => f.key === selectedFile)?.filename || ''}
          </div>
