...
import { runTests, runComponentTest } from '/js/services/foundry-testing.js';
...
export function useTestExecution(core, projectId, orSettings, session, showToast) {
  ...
  async function handleRunTests() {
    ...
    if (testableComps.length === 0) {
      showToast?.(t('profile.foundry.test_no_testable'), true);
  ...
  async function handleTestFixRequest(componentId, action) {
    ...
    if (testFixRound >= MAX_FIX_ROUNDS) {
      showToast?.(t('profile.foundry.test_fix_round') + ' ' + MAX_FIX_ROUNDS + ' — ' + t('profile.foundry.test_fix_manual'), true);
      return;
    }
    ...
    showToast?.(t('profile.foundry.test_fix_round') + ' ' + (testFixRound + 1));
