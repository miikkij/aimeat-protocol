...
import { AppDataMap } from './apps-tab.datamap.js';
    ...
    ${!myApps ? html`<${Spinner} text=${t('profile.apps.loading')} />`
      ...
      : myApps.map(a => html`
        <div class="card">
          ...
          ${/* Where this app puts what, as one line, and it OPENS HERE. The list shows the stamp and
                fetches nothing: it runs to 169 apps on the production node, and a per-card fetch of
                a whole document is the shape that has taken a surface down here before. The full map
                is pulled only for the one card somebody actually opens. */ ''}
          <${AppDataMap} owner=${a.owner || session.owner} filename=${a.filename || a.name}
            stamp=${a.data_map} />
