      ...
      <div class="pf-gen-edit-panel">
        ...
        <div class="pf-gen-actions">
          ...
          ${orSettings?.hasApiKey && html`
            <button class="btn-outline btn-sm pf-gen-or-run-btn ${edit.aiRunning === 'impact' ? 'pf-gen-or-running' : ''}"
    ...
    return html`
      <div class="pf-gen-edit-panel">
        ...
        <div class="pf-gen-actions">
          ...
          <button class="btn-outline btn-sm" onClick=${() => edit.setEditMode('request')}>${t('profile.generator.back')}</button>
        </div>
      </div>
    `;
  }
  ...
  if (edit.editMode === 'editing' && edit.impactParsed) {
    return html`
      <div class="pf-gen-edit-panel">
        ...
        ${edit.impactParsed.summary && html`<p class="pf-gen-subtitle">${edit.impactParsed.summary}</p>`}
              ...
              <div class="pf-gen-impact-item ${impactClass}">
                ...
                ${(a.impact === 'root' || a.impact === 'update') && comp && html`
                  <div class="pf-gen-actions mt-xs">
                    ...
                    ${orSettings?.hasApiKey && html`
                      <button class="btn-outline btn-sm pf-gen-or-run-btn ${edit.aiRunning === comp.id ? 'pf-gen-or-running' : ''}"
        ...
        <div class="pf-gen-actions">
          ${orSettings?.hasApiKey && edit.impactParsed.analysis.some(a => a.impact === 'root' || a.impact === 'update') && html`
            ...
            `}
          `}
          <button class="btn-outline btn-sm" onClick=${() => edit.setEditMode('impact')}>${t('profile.generator.backToImpact')}</button>
          <button class="btn-ghost btn-sm" onClick=${edit.exitEditMode}>${t('profile.generator.done')}</button>
        </div>
      </div>
    `;
  }
