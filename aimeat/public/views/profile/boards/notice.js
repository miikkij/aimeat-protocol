 ...
 * @version-history
 *   v2.1.0 -- 2026-09-22 -- The set's newer props: the notice body, the replies and the poster's
 *     standing keep their line breaks through Text lines (no <br> helper), the thanks chip is coral,
 *     Delete is in the danger tone, and the jump to the reply box uses the set's scrollToId.
...
import { Section, Fold, Stack, ListRow, Field, Action, Chip, Text, scrollToId } from '/components/poster-parts.js';
...
export function renderNotice(ctx, b, postId) {
  ...
  const chips = html`<${Stack} direction="wrap" density="compact">
    ...
    <${Chip} tone=${thanks ? 'coral' : 'muted'}>${c('thanksN', { n: thanks })}<//>
  ...
  const doors = html`
    ...
    <${Action} onClick=${() => scrollToId('bp-reply')}>${c('reply')}<//>
    <${Action} onClick=${() => ctx.handleReport(post.id)}>${c('report')}<//>`;
  const rail = html`<${Stack} density="compact">
    <${Text} kind="label">${c('poster')}<//>
    <${Text}>→ ${w.label}<//>
    <${Text} kind="caption" lines>${standingWords(standing)}${standing?.since ? '\n' + c('since', { d: day(standing.since) }) : ''}<//>
...
function toolsFold(ctx, boardId, post) {
  return html`<${Stack}>
    <${Stack} direction="wrap" align="center">
      ...
      <${Action} tone="danger" disabled=${ctx.updating} onClick=${() => ctx.handleDeletePost(boardId, post.id)}>${c('deleteNotice')}<//>
    <//>
