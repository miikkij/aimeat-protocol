/**
 * @file foundry.js
 * @description Foundry service — manages project state, task queue, and component
 *   lifecycle via the AIMEAT Memory API.
 *   All state stored at foundry.{projectId}.* memory keys.
 * @structure
 ...
 * @usage import { listProjects, createProject, registerComponent } from '/js/services/foundry.js';
 * @version-history
 ...
 *     translation i18n keys before removing foundry state
...
export async function listProjects() {
  const resp = await apiGet('/v1/memory?prefix=foundry.&owner_scope=true');
...
export async function getProject(projectId) {
  ...
  const resp = await apiGet(`/v1/memory?prefix=foundry.${projectId}.project&owner_scope=true`);
  const items = resp?.data?.items || [];
  const item = items.find(i => i.key === `foundry.${projectId}.project`);
  if (!item) return null;
...
export async function updateProject(projectId, updates) {
  ...
  await apiPut(`/v1/memory/foundry.${projectId}.project`, {
    value: updated,
    visibility: 'owner',
    version,
  });
...
export async function deleteProject(projectId, session) {
  ...
  // Phase 3: Delete all foundry state keys
  const resp = await apiGet(`/v1/memory?prefix=foundry.${projectId}.&owner_scope=true`);
  const items = resp?.data?.items || resp?.data?.entries || [];
...
/**
 * Write a debug artifact to disk for a foundry component.
...
export async function writeDebugArtifact(projectId, componentId, phase, content) {
  try {
    await apiPost(`/v1/foundry/${projectId}/debug/${componentId}`, { phase, content });
  } catch { /* best-effort */ }
}
...
export async function getInterviewSpec(projectId) {
  try {
    const key = `foundry.${projectId}.interview-spec`;
...
export async function savePendingEdit(projectId, data) {
  return apiPost('/v1/memory', {
    key: `foundry.${projectId}.pending-edit`,
    value: data,
    visibility: 'owner',
  });
}
...
export async function getPendingEdit(projectId) {
  try {
    const key = `foundry.${projectId}.pending-edit`;
    const resp = await apiGet(`/v1/memory?prefix=${key}&owner_scope=true`);
    const item = (resp?.data?.items || []).find(i => i.key === key);
    if (!item?.value) return null;
    return typeof item.value === 'string' ? JSON.parse(item.value) : item.value;
...
export async function clearPendingEdit(projectId) {
  try {
    await apiDelete(`/v1/memory/foundry.${projectId}.pending-edit`);
  } catch { /* ignore */ }
}
...
export async function saveProjectSettings(projectId, values, secretKeys) {
  return apiPost(`/v1/foundry/${projectId}/settings`, { values, secretKeys });
}
...
export async function getComponent(projectId, componentId) {
  const key = `foundry.${projectId}.component.${componentId}`;
  const resp = await apiGet(`/v1/memory?prefix=${key}&owner_scope=true`);
  const item = (resp?.data?.items || []).find(i => i.key === key);
  return item || null;
}
...
export async function saveComponent(projectId, component) {
  ...
  const key = `foundry.${projectId}.component.${data.id}`;
  if (version === 0) {
...
export async function enqueueTask(projectId, componentId, type, prompt, assignedTo, assignedToTag) {
  ...
  const entry = {
    ...
    assignedToTag: assignedToTag || 'foundry',
    status: 'pending',
    timeoutMinutes: 30,
    createdAt: new Date().toISOString(),
    expiresAt,
  ...
  await apiPost('/v1/memory', {
    key: `foundry.${projectId}.queue.${id}`,
    value: entry,
    visibility: 'owner',
  });
  return entry;
...
export async function pollResults(projectId) {
  const resp = await apiGet(`/v1/memory?prefix=foundry.${projectId}.results.&owner_scope=true`);
  const items = resp?.data?.items || resp?.data?.entries || [];
...
export async function pollLogs(projectId, taskIdVal) {
  try {
    const resp = await apiGet(`/v1/memory/foundry.${projectId}.logs.${taskIdVal}`);
    return resp?.data?.entries || [];
  } catch { return []; }
}

export async function checkQueueStatus(projectId) {
  const resp = await apiGet(`/v1/memory?prefix=foundry.${projectId}.queue.&owner_scope=true`);
  const items = resp?.data?.items || resp?.data?.entries || [];
  const now = Date.now();
  return items.map(i => {
    const val = typeof i.value === 'string' ? JSON.parse(i.value) : i.value;
...
export async function cleanupOldEntries(projectId) {
  ...
  for (const pfx of prefixes) {
    try {
      const resp = await apiGet(`/v1/memory?prefix=foundry.${projectId}.${pfx}.&owner_scope=true`);
      const items = resp?.data?.items || [];
      for (const item of items) {
        const val = typeof item.value === 'string' ? JSON.parse(item.value) : item.value;
