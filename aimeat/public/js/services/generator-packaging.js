/**
 * @file generator-packaging.js
 * @description Bridge between the Generator and Package/Template systems.
 *   Enables packaging generator projects into versioned bundles, importing packages
 *   back into the generator for editing, updating existing packages, forking, and
 ...
 *   - Import: importPackageToGenerator, parseGeneratorMetadata
 ...
 * @usage
 *   import { packageProject, importPackageToGenerator } from '/js/services/generator-packaging.js';
 * @version-history
 *   v1.0.0 — 2026-03-17 — initial implementation (Generator ↔ Package bridge)
 */
 ...
 * @param {*} result - Generator component result (raw AI output or extracted)
...
/**
 * Reverse normalization: convert package content string back to generator result format.
 * This is what gets stored in the generator component's `result` field.
 *
 * @param {string} type - Component type
 * @param {string} content - Package content string
 * @returns {*} - Generator-compatible result
 */
...
/* ── Extension result parser (copied from generator.js for independence) ── */
...
/**
 * Build a YAML manifest string from project metadata, components, and generator context.
 *
 * @param {object} project - Generator project
 * @param {object[]} pkgComponents - Normalized package components
 * @param {object} [generatorMeta] - Generator metadata to embed for round-trip fidelity
 * @returns {string} - YAML manifest string
 */
export function buildManifestYaml(project, pkgComponents, generatorMeta) {
  const manifest = {
  ...
  // Embed generator metadata for fork/edit round-trip
  if (generatorMeta) {
    manifest.generator = generatorMeta;
  }
...
/**
 * Reconstruct a blueprint from package components (when no generator metadata is available).
 */
...
/**
 * Build normalized package components array from generator components.
 * @param {object[]} components - Generator components with status 'done' and result
...
/**
 * Package a generator project into a bundle (first version).
 *
...
export async function packageProject(projectId, options = {}) {
  ...
  const manifest = buildManifestYaml(project, pkgComponents, generatorMeta);
...
/**
 * Compare generator components against the currently packaged version.
 * Returns a list of changes: added, modified, removed, unchanged.
 *
 * @param {object[]} generatorComponents - Current generator components (done + result)
 ...
 * @returns {Promise<string>} - New generator project ID
 */
export async function importPackageToGenerator(groupId, version, currentUser) {
  ...
  // Extract generator metadata from manifest
  ...
  // Create generator components from package components
