...
import { uiComponentsRouter } from '../routes/ui-components.js';
...
import { agentAiKeysRouter } from '../routes/agent-ai-keys.js';
...
import { mailTemplatesRouter } from '../routes/mail-templates.js';
...
export async function mountRoutes(
  ...
  app.use(signalsRouter(config, storage));                // Signals: hit collection (tracking image + JSON hit), owner reports
  ...
  app.use(mailTemplatesRouter(config, storage));
  ...
  // Decision rules first: `/v1/ai/decisions/stats` must be matched before `/v1/ai/decisions/:id`.
  app.use(decideRulesRouter(config, storage));
  app.use(decideRouter(config, storage));
  app.use(agentAiKeysRouter(config, storage));  // A key per agent, its daily cap and its gate switch
  ...
  // The operator's compliance surface: the node-wide report, the use-case register and the risk
