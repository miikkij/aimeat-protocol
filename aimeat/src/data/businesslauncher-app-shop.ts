...
<body class="bg-base-100 text-base-content min-h-screen flex flex-col">
  ...
  (function () {
    ...
    async function takeHold(it) {
      var msg = $('hold-msg');
      if (!session) {
        // AIMEAT.auth.signIn() is the interactive sign-in: the silent bridge first and then the
        // consent popup on an app origin, the sign-in modal anywhere else. This click is the user
        // gesture the popup needs. AIMEAT.auth.login() would only restore a session that already
        // exists. The 'login' handler in boot() repaints once the visitor is in. A node too old to
        // have signIn gets the pill's own button pressed, as before.
        msg.className = 'text-sm opacity-70';
        msg.textContent = t('signingIn');
        if (AIMEAT.auth.signIn) {
          AIMEAT.auth.signIn().catch(function () { return null; });
        } else {
