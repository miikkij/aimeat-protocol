...
<body class="bg-base-100 text-base-content min-h-screen flex flex-col">
  <!-- The bars run full width, their CONTENT shares the same max-w-4xl column as the page below.
       At page padding the title started at x=16 while the content it titled started at x=224. -->
  <nav class="navbar bg-base-200 shadow-sm">
    <div class="max-w-4xl mx-auto w-full px-4 flex items-center gap-2">
      ...
      <button id="agent-prompt" class="btn btn-outline px-3 text-sm" hidden>Copy agent prompt</button>
      <span id="login"></span>
    </div>
  </nav>

  <div id="tabs-row" class="w-full" hidden>
    <div id="tabs" class="tabs tabs-bordered max-w-4xl mx-auto px-4 pt-3">
  ...
  <!-- chrome-clear: the node's two permanent marks own the bottom strip, and this page has no footer
       to absorb them, so the last row of a long form would sit underneath. -->
  <main id="app" class="flex-1 w-full max-w-4xl mx-auto p-4 chrome-clear"></main>
  ...
  <!-- The workspace reads and writes go through this one. Without it AIMEAT.organism is undefined
       and the whole back office stops at "sign in", having signed the person in. -->
  ...
  (function () {
    ...
    async function start() {
      ...
      say('<h2 class="text-lg font-bold mb-3">Which shop?</h2>' + candidates.map(function (c, i) {
        return '<button class="btn btn-outline w-full justify-start mb-2 px-4" data-pick="' + i + '">' + esc(c.label) + '</button>';
      }).join(''));
    ...
    function renderSetup() {
      say('<h2 class="text-lg font-bold mb-2">One step before the shop can hold anything</h2>' +
        ...
        '<button class="btn btn-primary px-4 mt-3" id="copy-setup">Copy</button>');
    ...
    async function paint() {
      ...
      if (tab === 'enquiries') return paintEnquiries();
    }
    ...
    async function paintHome() {
      ...
      if (shopMeta && !shopMeta.contact) steps.push({ label: 'Open a way to get in touch', why: 'Until you do, the shop front shows no contact form and nothing can reach the Enquiries tab.', act: 'contact' });
      ...
      say('<h2 class="text-lg font-bold mb-1">What is left</h2>' +
        ...
        steps.map(function (s, i) {
          return '<div class="card bg-base-200 mb-3"><div class="card-body p-4 gap-1">' +
            ...
            '<div class="pt-2"><button class="btn btn-primary px-4" data-act="' + esc(s.act) + '">Do this</button></div>' +
            '</div></div>';
        }).join(''));
    ...
    async function doAct(act) {
      ...
      if (act === 'contact') {
        // Public Intake is the node's own anonymous-write path and the ONLY one. Defining the form
        // here pins the destination server-side: a submission is written as a record owned by this
        // shop, in this space, with only these three fields, whoever sends it and whatever else they
        // put in the body. The shop then stores a POINTER at the form, never a destination the front
        // could be talked into changing.
        try {
          var form = await AIMEAT.intake.defineForm({
            organism_id: CTX.org, ws: CTX.ws, namespace: 'shop.' + SPACES.leads,
            title: 'Get in touch',
            allowed_fields: ['name', 'email', 'message'],
            required_fields: ['message'],
            // The submitter never sends these: the node stamps them per submission, so every enquiry
            // has an id and a time even though the form asks for neither.
            defaults: { id: '{{uuid}}', receivedAt: '{{now}}' },
            // A field a person never sees and a robot fills in. Filled means dropped.
            honeypot_field: 'website',
            max_per_day: 200,
            mode: 'publish',
          });
          var saved = await AIMEAT.shop.admin(session, 'configure', {
            contact: { org: CTX.org, ws: CTX.ws, formId: form.form_id },
          });
          if (!saved || !saved.ok) throw new Error((saved && saved.error) || 'Could not save that.');
          shopMeta = await AIMEAT.shop.shop();
        } catch (err) {
      ...
      say('<h2 class="text-lg font-bold mb-1">' + esc(title) + '</h2>' +
        ...
        (itemsIn.length
          ? itemsIn.map(function (it) {
            // A row that says only its own id tells the owner nothing. Say what is in it: how many
            // products a catalogue holds, or the title of a page.
            var v = it.value || {};
            var what = v.title || v.name
              || (Array.isArray(v.items) ? v.items.length + ' products' + (v.currency ? ' · ' + v.currency : '') : '')
              || (Array.isArray(v.rows) ? v.rows.length + ' prices' : '');
            return '<div class="card bg-base-200 mb-2"><div class="card-body p-3 flex-row items-center gap-3">' +
              ...
              '<span class="flex-1 truncate">' + esc(what) + '</span>' +
              '</div></div>';
          }).join('')
          : '<div class="opacity-60">Nothing here yet. Your AI can fill this in — copy the agent prompt at the top.</div>') +
        (space === SPACES.catalog ? '<button id="publish" class="btn btn-primary px-4 mt-4">Publish to the shop front</button><div id="pub-msg" class="text-sm mt-2"></div>' : '') +
        (space === SPACES.policy ? '<button id="publish-pages" class="btn btn-primary px-4 mt-4">Publish the pages</button><div id="pub-msg" class="text-sm mt-2"></div>' : ''));
    ...
    /** What came in through the contact form, and the switch that opens it in the first place. */
    async function paintEnquiries() {
      var on = !!(shopMeta && shopMeta.contact);
      var index = await AIMEAT.organism.read(CTX.org, CTX.ws).catch(function () { return null; });
      var rows = ((index && index.spaces) || []).filter(function (s) { return s.name === SPACES.leads; })[0];
      var itemsIn = (rows && rows.items) || [];
      say('<h2 class="text-lg font-bold mb-1">Enquiries</h2>' +
        '<p class="opacity-70 text-sm mb-4">What came in through the contact form on the shop front.</p>' +
        (on
          ? ''
          : '<div class="card bg-base-200 mb-4"><div class="card-body p-4 gap-2">' +
            '<div class="font-semibold">The contact form is not open yet</div>' +
            '<div class="text-sm opacity-70">Turning it on puts a "Get in touch" button on the shop front. ' +
            'A message can be sent without an account, and it arrives here as a record you own.</div>' +
            '<div class="pt-1"><button id="open-contact" class="btn btn-primary px-4">Open the contact form</button></div>' +
            '</div></div>') +
        (itemsIn.length
          ? itemsIn.map(function (it) {
            var v = it.value || {};
            return '<div class="card bg-base-200 mb-2"><div class="card-body p-3 gap-1">' +
              '<div class="flex items-center gap-2 flex-wrap">' +
              '<span class="font-semibold">' + esc(v.name || 'Someone') + '</span>' +
              (v.email ? '<span class="opacity-70 text-sm">' + esc(v.email) + '</span>' : '') +
              '<span class="opacity-50 text-xs ml-auto">' + esc(String(v.receivedAt || '').slice(0, 16).replace('T', ' ')) + '</span>' +
              '</div>' +
              '<div class="whitespace-pre-wrap text-sm">' + esc(v.message || '') + '</div>' +
              '</div></div>';
          }).join('')
          : '<div class="opacity-60">' + (on ? 'Nothing has come in yet.' : '') + '</div>'));
      if ($('open-contact')) $('open-contact').onclick = function () { doAct('contact'); };
    }
        ...
        (itemsIn.length
          ? itemsIn.map(function (it) {
            return '<div class="flex items-center gap-3 mb-2">' +
              ...
              '</div>';
          }).join('') + '<button id="save-stock" class="btn btn-primary px-4 mt-3">Save</button><div id="stock-msg" class="text-sm mt-2"></div>'
