...
<!--
  ...
  @version-history
    ...
    v2.0.1 — 2026-09-14 — The poll re-renders only when the screen changed. Every poll used to
      restart the rotation at the first view, so a 30 s poll showed only the first 30 / interval views.
-->
<!DOCTYPE html>
<html lang="en">
<head>
...
<body>
  ...
  <script>
    ...
    // What the stage shows: the payload plus which views are inside their active window right now.
    // A poll that returns the same signature leaves the stage and the rotation alone; re-rendering
    // restarts the rotation at the first view, so a 30 s poll would show only 30 / interval views.
    function signature(payload) {
      const now = Date.now();
      const active = payload.views.map((v) => (!v.activeFrom || Date.parse(v.activeFrom) <= now)
        && (!v.activeUntil || Date.parse(v.activeUntil) >= now));
      return JSON.stringify(payload) + '|' + active.join(',');
    }

    // ── boot + poll ───────────────────────────────────────────────────────
    let shown = null;  // signature of what the stage shows; null after an error or before the first load
