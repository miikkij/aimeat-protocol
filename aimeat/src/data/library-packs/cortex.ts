...
export const CORTEX_PACKS: LibraryPack[] = [
  ...
  },
  {
    ...
    kind: 'cortex',
    category: 'core',
    title: 'Vocabulary lookup (Finto / YSO)',
    description: 'Look a concept up in a real vocabulary, and turn what the person picked into one they own',
    url: '/v1/cortex/aimeat-vocab/libs/aimeat-vocab.js',
    include: ['<script src="{{BASE_URL}}/v1/cortex/aimeat-vocab/libs/aimeat-vocab.js"></script>'],
    requires: ['aimeat-auth', 'aimeat-onto'],
    version: '1.0.0',
    license: 'MIT',
    apiSurface: 'AIMEAT.vocab',
    aiDoc: 'The OUTSIDE half of vocabularies; AIMEAT.onto is the half the owner keeps. The flow is search outside, let the person choose, save inside — after that the app reads the owner own record and never calls the service again, so it keeps working when that service is down and the vocabulary is theirs rather than borrowed. (1) const { results } = await AIMEAT.vocab.search("lintu", { lang: "fi" }) → [{ uri, prefLabel, lang, vocab }]; a trailing wildcard is added for you, so "lintu" finds "lintuharrastus". (2) const { concept } = await AIMEAT.vocab.concept(uri, { langs: ["fi","en"] }) → labels per language, the words that also mean it, and what sits above and below, for when the person needs to see the neighbourhood before choosing. (3) const { scheme } = await AIMEAT.vocab.scheme(chosenUris, { id: "hobbies", label: { fi: "Harrastukset", en: "Hobbies" }, langs: ["fi","en"] }), then await AIMEAT.onto.saveVocab("hobbies", scheme). Each saved concept keeps skos:exactMatch pointing back at the source concept — that is the field that lets two systems which never agreed on anything tell they mean the same thing, so do not strip it. The source is Finto (National Library of Finland) and YSO, about 30 000 concepts in fi/sv/en under CC BY 4.0; the scheme comes back carrying dcterms:rights and dcterms:source, and the credit has to stay with the record and be shown where a person sees the concepts. Languages are fi, sv and en only; ask for the ones you render, because each one is a separate lookup. A refusal is a thrown Error carrying the code (QUERY_TOO_SHORT, BAD_URI, BAD_ID, NO_CONCEPTS, TOO_MANY, FINTO_UNREACHABLE, FINTO_ERROR) — show it rather than turning it into "something went wrong". Needs a signed-in session; reaches the vocab-finto extension, never /v1/ext/ from your own code.',
    changelog: [],
    tierHint: 'T2',
    interviewTriggers: ['vocabulary', 'concepts', 'taxonomy', 'tags', 'aiheet', 'asiasanat', 'sanasto', 'ontology', 'yso', 'finto'],
    sizeEstimate: '~4KB',
    status: 'preview',
    modelTier: 'needs-doc',
    promptLine: '- aimeat-vocab — look a concept up in a real vocabulary (Finto/YSO) and save what was picked as your own (`AIMEAT.vocab`)',
  },
];
