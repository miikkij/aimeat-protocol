...
export const V2_EXCLUDED: readonly string[] = [
    ...
    // Added 2026-08-15, and this entry is a promise rather than a decision. Both belong on the agent
    // surfaces: adding a co-owner is exactly the kind of thing a person's own agent should be able to
    // do for them. Placing them there requires a connector implementation, and registering one grows
    // the published `aimeat` npm surface, which is a RELEASE — the operator's call, not a side effect
    // of this change. Until that release they live on the node's own /v1/mcp and on REST. Remove
    // these two lines, register them in cli/connect/mcp/tools/organisms.ts, and flip connectorMcp to
    // true in the catalog, all in the same commit as the version bump.
    'aimeat_organism_owner_add', 'aimeat_organism_owner_remove',
];
