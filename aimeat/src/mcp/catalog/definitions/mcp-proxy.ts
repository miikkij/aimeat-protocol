...
export const mcpProxyTools: AimeatToolDefinition[] = [
    ...
    },
    {
        ...
        input: {
            ...
            group: { type: 'string', description: "Attach it to a GROUP instead of to this person, so the group's members reach it without anybody handing out a token. Only an owner or an admin of the group may; using what is attached needs only membership." },
            ws: { type: 'string', description: "With group, bind it to ONE workspace inside that group. Then the workspace's own roles decide: a contributor may call it, a viewer only sees it is there, and a member of the group with no role in that workspace reaches nothing." },
