...
export async function createEmailInvitation(
  ...
  if (emailSvc?.enabled) {
    emailSent = await emailSvc.sendInvite(cleanEmail, {
      ...
      // expressed no preference, and the ISO date is the honest answer for it.
      expiresLabel: existing
        ? formatForPerson(
          { locale: existing.locale ?? null, region: existing.region ?? null, timezone: existing.timezone ?? null },
 ...
 *
 * Moved to invitations-agent-door.ts on 2026-09-12, when this file crossed 800 lines. It is the
 * same record, the same token and the same accept endpoint as an organism invitation, so the
 * minting helpers above are what it imports; only the door differs. */
