 ...
 *     asked for a proposal at the end of a build say this instead.
...
export const ATELIER_MOSAIC_IN_GENRE = 'IN A GENRE FORK, THE WORKING SCREEN IS A MOSAIC INSIDE THE GENRE. The genre is the frame: its masthead, its physics, its look. '
  ...
  + '{ "took":   [ { "part": "leiska-work-queue", "why": "jobs wait, get taken and get done: a queue with states is this app" } ],\n'
  + '  "passed": [ { "part": "leiska-dashboard", "why": "it leads with numbers, and here the list is the work" } ],\n'
  + '  "made":   [ { "name": "shift-strip", "what": "one row of the day\'s shifts, tappable", "why": "the Book has no block for a day cut into named spans" } ] }\n'
  + '</script>\n'
  + '```\n\n'
  + '`took` is a part that is in this page and what made it fit (a part named here needs no `aimeat-book-parts` meta). `passed` is a part you looked at and left, and what made it NOT fit: it is the only place a part\'s weakness is ever written down, so it is the most useful of the three. `made` is what you built by hand because the Book had nothing, and why it had nothing. '
  + 'Write the true reason, including "the name matched and I did not look further": a reason that sounds good and is not why you chose teaches the Book the wrong thing. The publish keeps these with the version they belong to. '
  + 'NOTHING GOES INTO THE BOOK BECAUSE A BUILD IS FINISHED. An app is often rebuilt twice before its owner likes it, and a version that was thrown away must not teach the Book anything. '
  + 'Tell the owner, in one line when you hand the app over, that when they are happy with it they can say so; that is recorded with `aimeat_designbook_keep { filename }`, by whichever AI they say it to, and it is the moment what this app took and made starts to count.\n\n';
