...
function checkMarkup(html: string, prefix: string): void {
  if (/<!--/.test(html)) refuse('A component\'s markup carries no comments: say what it is in `use`.');
  for (const tag of html.matchAll(/<\s*(\/?)\s*([a-zA-Z][\w:-]*)([^>]*)>/g)) {
    ...
    if (tag[1]) continue;
    for (const attr of tag[3].matchAll(/([^\s"'=<>/]+)(?:\s*=\s*("[^"]*"|'[^']*'|[^\s"'=<>`]+))?/g)) {
      ...
      if (/^\s*(javascript|data|vbscript)\s*:/i.test(value) || /url\s*\(/i.test(value)) {
...
function checkStyles(css: string, prefix: string): void {
  ...
  if (/url\s*\(/i.test(css)) refuse('A component\'s stylesheet carries no url(): it loads nothing, and a picture is the app\'s to add.');
  if (/expression\s*\(|behavior\s*:|-moz-binding/i.test(css)) refuse('A component\'s stylesheet carries no expression(), behavior or binding.');
  if (/!important/i.test(css)) refuse('A component\'s stylesheet carries no !important: it lands inside somebody else\'s page and must lose to it where they disagree.');
  if (/position\s*:\s*(fixed|sticky)/i.test(css)) refuse('A component stays where the app puts it: no position: fixed or sticky.');
  if (/animation[^;{}]*\binfinite\b/i.test(css)) refuse('A component does not move at idle: no infinite animation. An entrance that ends is fine, and the ambient is the one layer allowed to keep moving.');
  // A literal colour cannot follow the page it lands in. Inside var(--ak-x, #fallback) it is a fallback, which is fine.
  const bare = css.replace(/var\(\s*--[\w-]+\s*,[^()]*(?:\([^()]*\)[^()]*)*\)/g, 'var()');
  ...
  const rules = css.replace(/\/\*[\s\S]*?\*\//g, '').replace(/@(?:media|supports|container|layer)[^{]*\{/g, '').replace(/\{[^{}]*\}/g, '\n');
  for (const selector of rules.split(/[\n,]/).map(s => s.trim()).filter(s => s && s !== '}')) {
    if (/^(from|to|\d+%)$/.test(selector) || selector.startsWith('@keyframes')) continue;
