/**
 * @file foundry-debug.ts
 * @description Debug artifact writer for the foundry pipeline — saves prompts,
 *   generated code, test results, and logs to disk for easy debugging.
 *   Files are written to data/debug/foundry/<projectId>/ in a structured
 *   folder layout with one subfolder per component.
 * @structure
 *   - FoundryDebugWriter: per-project writer with methods for each artifact type
 ...
 *   const debug = new FoundryDebugWriter(projectId);
 *   await debug.writeComponentPrompt('ext-1', promptText);
 * @version-history
 *   v1.0.0 — 2026-03-26 — Copied from generator-debug.ts (v1.0.0) and renamed to foundry
 */
...
const DEBUG_BASE = join(process.cwd(), 'data', 'debug', 'foundry');
 ...
 */
export class FoundryDebugWriter {
  ...
  private async safeWrite(filePath: string, content: string): Promise<void> {
    try {
      ...
      console.error(`[foundry-debug] Failed to write ${filePath}:`, (e as Error).message);
    }
  }
  ...
  async appendLog(entry: Record<string, unknown>): Promise<void> {
    ...
    try {
      ...
      console.error(`[foundry-debug] Failed to append log:`, (e as Error).message);
    }
  }
