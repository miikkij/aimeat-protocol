/**
 * @file src/services/foundry-registration.ts
 * @description Internal registration helpers used by the foundry pipeline.
 ...
 * @version-history
 *   v1.0.0 — 2026-03-26 — Copied from generator-registration.ts (v1.1.0) and renamed to foundry
 */
...
export async function registerCsm(content: string, ownerName: string, storage: Storage): Promise<void> {
  ...
  // If CSM already exists, remove it first (foundry always overwrites)
...
export async function registerMsm(content: string, ownerName: string, storage: Storage): Promise<void> {
  ...
  // If MSM already exists, remove it first (foundry always overwrites)
 ...
 * Content is the YAML+JS composite string produced by the foundry.
...
export async function registerExtension(content: string, ownerName: string, ownerGhii: string, storage: Storage, maxExtensionsPerOwner?: number): Promise<void> {
  ...
  // The foundry stores YAML at the top, followed by ```javascript ... ``` blocks.
  ...
  // If extension already exists, remove it first (foundry always overwrites)
 ...
 * The foundry stores raw HTML; we accept both base64 and raw.
