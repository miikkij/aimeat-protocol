 ...
 *   v2.6.0 — 2026-08-31 — The CONTRACT — MemoryWriteResult, ExtensionCtx, EXT_HASH_REFERENCE_JS,
 *     ExtensionLimits — moved to extension-ctx-contract.ts, and is re-exported from here so no
 *     importer had to change. A pure move, made because this file crossed the 800-line ceiling; what
 *     stayed is how the runtime is built, guarded and torn down, which is a different subject from
 *     what a script sees.
 *   v2.5.0 — 2026-08-31 — `ctx.ai.start`: a BACKGROUND model call with a handle. The sandbox caps at
 *     60 seconds and holds a QuickJS runtime for the whole wait, so a twenty-minute generation had
 *     nowhere to happen. What moves out is the waiting, not the work: the extension starts a job,
 *     returns in milliseconds, and the job calls one of the extension's own actions when the answer
 *     has landed. Billed to the INSTALLER, like ctx.buy, and absent on a road that does not know the
 *     record — there is no honest answer there to who pays.
...
// The contract a script is written against lives in its own file (the 800-line ceiling), and is
// re-exported here so every existing import keeps working unchanged.
export type { MemoryWriteResult, ExtensionCtx, ExtensionLimits } from './extension-ctx-contract.js';
export { EXT_HASH_REFERENCE_JS } from './extension-ctx-contract.js';
import type { ExtensionCtx, ExtensionLimits } from './extension-ctx-contract.js';
import { EXT_HASH_REFERENCE_JS } from './extension-ctx-contract.js';
...
(async () => {
    ...
    const ctx = {
        ...
        // Start a background model call. JSON both ways, same as buy: the bridge moves strings, and
        // the answer is a decision object rather than a throw so a full queue is something the
        // script can handle instead of a failed run.
        ai: __ai_start ? {
            start: async (opts) => JSON.parse(await __call(__ai_start, [JSON.stringify(opts ?? {})])),
        } : undefined,
        consent: {
...
export async function executeExtensionAction(
    ...
    try {
        ...
        // ── Background AI jobs ────────────────────────────────
        // Starting one is cheap and returns at once; it is the WAITING that had to leave the sandbox.
        // Counted against maxApiCalls like any other outbound step, because it is one.
        registerAsyncHostFn(vm, '__ai_start',
            ctx.ai
                ? async (optsJson) =>
                    JSON.stringify(await ctx.ai!.start(JSON.parse(optsJson || '{}') as Parameters<NonNullable<ExtensionCtx['ai']>['start']>[0]))
                : null,
            counter, limits.maxApiCalls, inflight);
