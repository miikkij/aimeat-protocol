/**
 * @file src/services/hello-integration-prompt.ts
 * @author Jouni Miikki
 * SPDX-License-Identifier: MIT
 * @description The Hello Integration instruction an owner pastes into a newly connected agent, and
 *   the tool sequence it walks. ONE text, served three ways: printed by `aimeat connect`, written
 *   into the skill bundle's BUNDLE.md, and answered by GET /v1/prompts/hello-integration, which is
 *   what the profile's Agents tab shows.
 *
 *   WHY IT MOVED HERE. It lived in src/cli/connect/onboarding-prompt.ts, where a route cannot
 *   reach it, so the browser kept a hand copy in public/views/profile/agents/connect-prompts.js.
 *   The instruction review of 2026-09-18 found the two had drifted: v1.1.0 of the CLI text gained
 *   the rule that only five aimeat_onboarding_* tools exist and the stop condition
 *   summary.completable, and the browser copy got neither, so an agent onboarded from the web page
 *   was the one still inventing tool names. The text below is the CLI's, moved byte for byte.
 * @structure HELLO_INTEGRATION_TOOL_SEQUENCE · buildAgentOnboardingInstruction()
 * @usage
 *   import { buildAgentOnboardingInstruction } from '../services/hello-integration-prompt.js';
 * @version-history
 *   v1.0.0 — 2026-09-18 — Moved verbatim from src/cli/connect/onboarding-prompt.ts (its v1.2.0).
 */
