...
const CLASSIFY_SYSTEM = 'You are a librarian that files a user\'s note into the right place in their knowledge structure. You only ever choose ids that appear in the provided structure. You always answer with a single JSON object and nothing else.';

function buildPrompt(context: PlacementOrganism[], text: string): string {
  return [
    'Here is the user\'s knowledge structure (organisms, each with workspaces, each with document spaces). Use ONLY these exact ids:',
    '',
    '```json',
    JSON.stringify({ organisms: context }, null, 2),
    '```',
    '',
...
export async function classifyNote(
  ...
  let result;
  try {
    result = await complete(decryptedKey, model, buildPrompt(context, text), CLASSIFY_SYSTEM, baseUrl, { temperature: 0.2 });
  } catch (e) {
