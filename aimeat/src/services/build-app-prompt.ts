 ...
 *     stores nothing, so the paragraph opens with "declare the space before you write into it" and
 *     says what the refusal carries and that the SDK throws it. The rest of the heal step is unchanged.
...
function composeAppPrompt(
  ...
  body += '\n';
  body += '**Four rows nobody draws, and they are the ones an audit lands on.** The table above is what the node holds and can hand back. These are not, and each answers *can you delete it* differently:\n';
  body += '\n';
  body += '| What | Where the real one is | Who decides about it | What deleting it means |\n';
  body += '|---|---|---|---|\n';
  body += '| work with no final state yet: a draft, a half-finished send, a hold, an approval waiting | your workspace, but unsettled | the group | delete it, and say what happens to the work it was part of |\n';
  body += '| a copy of somebody else\'s record: a registry lookup, an enrichment, a cached profile | THEIR system | them, not you | your copy goes; theirs does not, and it may come back on the next fetch |\n';
  body += '| what has already left: an email delivered, a webhook posted, a file downloaded | the recipient\'s inbox, the other system | nobody you can reach | you cannot delete it. All you hold is the record that it happened |\n';
  body += '| a credential for a system you do not run: an SMTP password, an API key | your store, encrypted | the person who set it up | delete it and the connection stops; the other system is unaffected |\n';
  body += '\n';
  body += '**Say which state a thing is in rather than showing it as settled.** In-flight work is the commonest one to get wrong: write each outcome as it happens rather than all of them at the end, so a closed tab loses the remaining work and not the record of what already went. Then say in the UI what a person is looking at — finished, still running, or given up on. A row that renders identically in all three is a lie the person will repeat.\n';
  body += '**A copy of somebody else\'s record carries the date it was fetched, on screen.** It goes stale by definition, nobody tells you when it changes, and a stale figure shown as current is how an app makes a decision worse rather than better. Re-fetch on read where that is cheap; where it is not, show the age and let a person ask for a fresh one.\n';
  body += '**What has left the node is the sharpest row on the map.** An erasure request cannot reach a message delivered last week, so decide BEFORE you build what you keep about it: an opaque token instead of an address, a hash instead of the value, the fact of a send instead of its content. Deliberate non-holding is the answer to that question, and it only exists if you chose it early.\n';
  body += '**A stand-in you cannot reverse is a design decision worth naming.** An opaque token only the sender can map back to a person; a hash that answers "is this the same address" without storing the address. These let a feature work while the node stays unable to identify anybody. Put them on the map as what they are, because from the outside they look like identifiers and somebody will ask.\n';
  body += '\n';
  ...
  // Last item on the list on purpose: it is the one that costs nothing now and costs somebody hours
  // later, so it belongs where the work is being declared finished rather than in the middle.
  body += '7. **Write the data map** (see its section above) if the app stores anything at all. It is the only place the next person or AI can read where this app puts what and why, and without it they will read your source, give up, and put their new thing wherever they can reach. Check your own work against it: if the `form` you declared and the places your rows actually land disagree, the storage is wrong, not the map.\n\n';
