...
function showLoginModal(opts, renderBtn) {
  ...
  modal.innerHTML = '<style>.aimeat-inp:focus{border-color:#E8564A !important;outline:none}.aimeat-go{flex:1;padding:10px;background:#E8564A;color:#fff;border:none;border-radius:8px;cursor:pointer;font-weight:700;font-size:14px;letter-spacing:.3px;box-shadow:0 2px 8px rgba(232,86,74,.25);transition:transform .15s,box-shadow .15s,background .15s}.aimeat-go:hover{background:#d4493f;transform:translateY(-1px);box-shadow:0 4px 14px rgba(232,86,74,.35)}.aimeat-label{display:block;margin-bottom:4px;font-size:11px;font-weight:600;letter-spacing:.5px;text-transform:uppercase;color:#6B7280}</style>'
    ...
    + '<p style="margin:0 0 12px;font-size:13px;color:#6B7280;line-height:1.5">' + escHtml(i.descNew || 'New? Pick a username and password to create an account.') + ' ' + escHtml(i.descReturning || 'Already have an account? Enter your username and password.') + '</p>'
    + '<label class="aimeat-label">' + escHtml(i.usernameLabel || 'Username') + '</label>'
    + '<input id="aimeat-username" class="aimeat-inp" placeholder="' + escHtml(i.usernamePlaceholder || 'Username') + '" style="width:100%;padding:10px 12px;background:#F9FAFB;border:1px solid #E5E7EB;border-radius:8px;color:#1A1A2E;font-size:14px;margin-bottom:12px;box-sizing:border-box;transition:border-color .2s">'
    + '<label class="aimeat-label">' + escHtml(i.passwordLabel || 'Password') + '</label>'
    + '<input id="aimeat-password" type="password" class="aimeat-inp" placeholder="' + escHtml(i.passwordPlaceholder || 'Password (min 4 chars)') + '" style="width:100%;padding:10px 12px;background:#F9FAFB;border:1px solid #E5E7EB;border-radius:8px;color:#1A1A2E;font-size:14px;margin-bottom:12px;box-sizing:border-box;transition:border-color .2s">'
    + '<label class="aimeat-label">' + escHtml(i.displayNameLabel || 'Display Name') + ' <span style="font-weight:400;text-transform:none;letter-spacing:0">(' + escHtml(i.displayNameHint || 'optional, for new accounts') + ')</span></label>'
    + '<input id="aimeat-displayname" class="aimeat-inp" placeholder="' + escHtml(i.displayNamePlaceholder || 'Display Name') + '" style="width:100%;padding:10px 12px;background:#F9FAFB;border:1px solid #E5E7EB;border-radius:8px;color:#1A1A2E;font-size:14px;margin-bottom:16px;box-sizing:border-box;transition:border-color .2s">'
    + '<div style="display:flex;gap:8px;align-items:center">'
    + '<button id="aimeat-go-btn" class="aimeat-go">' + escHtml(i.signInBtn || 'Sign In') + '</button>'
    + '<button id="aimeat-cancel-btn" style="padding:8px 16px;background:#F3F4F6;color:#1A1A2E;border:1px solid #E5E7EB;border-radius:8px;cursor:pointer;font-size:14px;font-weight:500">' + escHtml(i.cancelBtn || 'Cancel') + '</button>'
    + '</div>'
    ...
    + '<p style="margin:0 0 6px;font-size:13px;font-weight:600;color:#1A1A2E">' + escHtml(i.whyTitle || '\\u2728 What do you get?') + '</p>'
    + '<ul style="margin:0;padding:0 0 0 18px;font-size:12px;color:#6B7280;line-height:1.8;list-style:none">'
