...
`;
  ...
  // ── DELETE /v1/admin/extensions/available/:name/actions/:actionId — Remove action from disk extension ──
  router.delete('/v1/admin/extensions/available/:name/actions/:actionId', requireAuth(), requireRole('operator'), async (req, res) => {
    try {
      const name = req.params.name as string;
      const actionId = req.params.actionId as string;
      const dir = getBundledExtensionsDir();
      ...
      if (idx === -1) {
        res.status(404).json(error(config.nodeId, 'NOT_FOUND', `Action "${actionId}" not found`));
        return;
      }
      ...
      const scriptPath = join(extDir, 'actions', `${actionId}.js`);
      if (existsSync(scriptPath)) unlinkSync(scriptPath);

      logger.info(`Action removed: ${name}/${actionId}`, { by: req.auth!.sub });
      res.json(success(config.nodeId, { id: actionId, deleted: true }));
    } catch (err) {
      logger.error('Failed to delete action', { error: (err as Error).message });
      res.status(500).json(error(config.nodeId, 'INTERNAL_ERROR', 'Failed to delete action'));
    }
  });

  // ── GET /v1/admin/extensions/available/:name/manifest — Read extension.yaml ──
  router.get('/v1/admin/extensions/available/:name/manifest', requireAuth(), requireRole('operator'), async (req, res) => {
    try {
      const name = req.params.name as string;
      const dir = getBundledExtensionsDir();
      const yamlPath = join(dir, name, 'extension.yaml');
      ...
      res.json(success(config.nodeId, { manifest: content }));
    } catch (err) {
      logger.error('Failed to read manifest', { error: (err as Error).message });
      res.status(500).json(error(config.nodeId, 'INTERNAL_ERROR', 'Failed to read manifest'));
    }
  });

  // ── PUT /v1/admin/extensions/available/:name/manifest — Write extension.yaml ──
  router.put('/v1/admin/extensions/available/:name/manifest', requireAuth(), requireRole('operator'), async (req, res) => {
    try {
      const name = req.params.name as string;
      const manifest = (req.body as Record<string, unknown>).manifest as string;

      if (!manifest || typeof manifest !== 'string') {
        res.status(400).json(error(config.nodeId, 'VALIDATION_ERROR', 'manifest (YAML string) is required'));
        return;
      }

      // Validate it's parseable YAML
      try { parseYaml(manifest); } catch {
        res.status(400).json(error(config.nodeId, 'VALIDATION_ERROR', 'Invalid YAML syntax'));
        return;
      }
      ...
      const { writeFileSync: wfs } = await import('node:fs');
      wfs(join(extDir, 'extension.yaml'), manifest, 'utf-8');

      logger.info(`Manifest saved: ${name}/extension.yaml`, { by: req.auth!.sub });
      res.json(success(config.nodeId, { saved: true }));
    } catch (err) {
      logger.error('Failed to write manifest', { error: (err as Error).message });
      res.status(500).json(error(config.nodeId, 'INTERNAL_ERROR', 'Failed to write manifest'));
    }
  });
