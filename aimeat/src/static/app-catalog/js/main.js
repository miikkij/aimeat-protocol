...
import { I18N } from './i18n-data.js';
  ...
  var pbExtensionsCache = null; // cached cortex extension list
  var pbExtDetailCache = {};   // cached extension details (name → detail object)
  ...
  function loadPbExtensions() {
    var list = document.getElementById('pb-ext-list');
    var config = loadConfig();
    if (!config.aimeatUrl) {
      list.innerHTML = '<span style="color:var(--text-muted);font-size:.8rem">Configure AIMEAT URL in Settings to load extensions</span>';
      return;
    }

    // Use cached data if available
    if (pbExtensionsCache) {
      renderPbExtensions(pbExtensionsCache);
      return;
    }

    list.innerHTML = '<span style="color:var(--text-muted);font-size:.8rem">Loading extensions...</span>';
    var url = config.aimeatUrl.replace(/\/+$/, '');

    var token = cortexToken;
    var fetchExts = function(authToken) {
      fetch(url + '/v1/cortex?status=active', {
        headers: { 'Authorization': 'Bearer ' + authToken }
      })
        .then(function(r) { return r.json(); })
        .then(function(data) {
          if (!data || !data.data || !data.data.extensions) {
            list.innerHTML = '<span style="color:var(--text-muted);font-size:.8rem">No extensions available</span>';
            return;
          }
          pbExtensionsCache = data.data.extensions;
          renderPbExtensions(pbExtensionsCache);
        })
        .catch(function() {
          list.innerHTML = '<span style="color:var(--text-muted);font-size:.8rem">Failed to load extensions</span>';
        });
    };
    ...
    if (token) {
      ...
      fetch(url + '/v1/auth/anonymous', { method: 'POST' })
        .then(function(r) { return r.json(); })
        .then(function(authData) {
          var t = authData.data && authData.data.token;
          if (!t) { list.innerHTML = '<span style="color:var(--text-muted);font-size:.8rem">Auth failed</span>'; return; }
          cortexToken = t;
          fetchExts(t);
        })
        .catch(function() {
          list.innerHTML = '<span style="color:var(--text-muted);font-size:.8rem">Failed to authenticate</span>';
        });
    }
  }

  function renderPbExtensions(extensions) {
    var list = document.getElementById('pb-ext-list');
    if (!extensions.length) {
      list.innerHTML = '<span style="color:var(--text-muted);font-size:.8rem">No active extensions</span>';
      return;
    }
    list.innerHTML = extensions.map(function(ext, i) {
      var types = (ext.component_types || []).join(', ');
      return '<label class="pb-ext-chip" data-ext-index="' + i + '">'
        + '<input type="checkbox" data-ext-name="' + cortexEsc(ext.name) + '"> '
        + cortexEsc(ext.short_name || ext.name)
        + ' <span style="font-size:.7rem;color:var(--text-muted)">(' + cortexEsc(types) + ')</span>'
        + '</label>';
    }).join('');

    // Toggle selected class and update preview on change
    list.querySelectorAll('input[type="checkbox"]').forEach(function(cb) {
      cb.addEventListener('change', function() {
        this.closest('.pb-ext-chip').classList.toggle('selected', this.checked);
        if (this.checked) {
          fetchExtDetail(this.getAttribute('data-ext-name'));
        } else {
          updatePbPreview();
        }
      });
  ...
  function getSelectedExtensions() {
    var selected = [];
    var checkboxes = document.querySelectorAll('#pb-ext-list input[type="checkbox"]:checked');
    checkboxes.forEach(function(cb) {
      var name = cb.getAttribute('data-ext-name');
      // Prefer detail cache, fall back to list data
      if (pbExtDetailCache[name]) {
        selected.push(pbExtDetailCache[name]);
      } else if (pbExtensionsCache) {
        for (var i = 0; i < pbExtensionsCache.length; i++) {
          if (pbExtensionsCache[i].name === name) {
            selected.push(pbExtensionsCache[i]);
            break;
          }
        }
      }
  ...
  function fetchExtDetail(extName) {
    // Already cached
    if (pbExtDetailCache[extName]) { updatePbPreview(); return; }
    var config = loadConfig();
    if (!config.aimeatUrl || !cortexToken) { updatePbPreview(); return; }
    var url = config.aimeatUrl.replace(/\/+$/, '');
    fetch(url + '/v1/cortex/' + encodeURIComponent(extName), {
      headers: { 'Authorization': 'Bearer ' + cortexToken }
    })
      .then(function(r) { return r.json(); })
      .then(function(data) {
        if (data && data.data) {
          pbExtDetailCache[extName] = data.data;
        }
        updatePbPreview();
      })
      .catch(function() { updatePbPreview(); });
  }
  ...
  document.addEventListener('DOMContentLoaded', function () {
    ...
    function buildAiPrompt(appName, code) {
      var config = loadConfig();
      var nodeUrl = config.aimeatUrl ? config.aimeatUrl.replace(/\/+$/, '') : 'https://your-aimeat-node.example';

      return 'Here is an HTML app called "' + appName + '".\n' +
        'Make the following change: [DESCRIBE YOUR CHANGE HERE]\n\n' +
        'Return the COMPLETE updated HTML file. Do not omit any parts.\n\n' +
        '## AIMEAT Platform Instructions\n\n' +
        'This app runs in the AIMEAT ecosystem. Here is what you need to know:\n\n' +
        '### Available Client Libraries\n' +
        'Load with <script src> from ' + nodeUrl + '/v1/libs/. Load aimeat-auth first; use only what you need:\n\n' +
        'Core: aimeat-auth.js (login/session, `AIMEAT.auth`), aimeat-data.js (key-value memory + search, `AIMEAT.data`), aimeat-storage.js (files, `AIMEAT.storage`).\n' +
        'AI: aimeat-ai.js — LLM completions on the USER\'s own OpenRouter key (`AIMEAT.ai.complete`, requires aimeat-auth).\n' +
        'Social & economy: aimeat-social.js (boards), aimeat-wallet.js (morsels), aimeat-work.js (actions), aimeat-agents.js (commission agents), aimeat-capabilities.js (shared capabilities).\n' +
        'Media & misc: aimeat-audio.js, aimeat-speech.js, aimeat-header.js, aimeat-tunnel.js.\n' +
        'Real-time multiplayer: no lib — POST/GET ' + nodeUrl + '/v1/realtime/rooms then WebSocket /v1/realtime/ws?room=ID (ICE config at /v1/realtime/ice-servers).\n\n' +
        '### Auth Pattern\n' +
        'Handle BOTH login paths: a fresh sign-in click (onLogin) AND a page that loads already signed in (restore the session yourself). `onLogin` fires ONLY on a fresh sign-in — NOT on reload when a session already exists.\n' +
        '```html\n' +
        ...
        'AIMEAT.auth.mountLoginButton("#login", { onLogin: showApp, onLogout: hideApp });\n' +
        '// Already signed in on load? onLogin will NOT fire — restore the session yourself:\n' +
        'AIMEAT.auth.login().then(function (session) { if (session) showApp(session); });\n' +
        '</' + 'script>\n' +
        '```\n\n' +
        '### Data Storage\n' +
        '```javascript\n' +
        'await AIMEAT.data.set("key", "value", { visibility: "private", tags: ["t1"] });\n' +
        'var val = await AIMEAT.data.get("key");\n' +
        'var results = await AIMEAT.data.search("query");\n' +
        '```\n\n' +
        '### Upload to AIMEAT Node\n' +
        'After updating, the user can upload the app to their AIMEAT node:\n' +
        '```\n' +
        'POST ' + nodeUrl + '/v1/apps\n' +
        'Authorization: Bearer <jwt>\n' +
        'Content-Type: application/json\n' +
        '{ "filename": "app.html", "content": "<base64>", "mime_type": "text/html" }\n' +
        '```\n\n' +
        ...
        '  var a = document.createElement("a");\n' +
        '  a.href = URL.createObjectURL(blob); a.download = "my-app.html"; a.click();\n' +
        '}\n' +
        '```\n\n' +
        '### Browser APIs Available\n' +
        'Canvas, WebGL, Web Audio, WebRTC, Camera, Geolocation, LocalStorage, IndexedDB,\n' +
        'Notifications, Drag & Drop, Clipboard, Speech API, Fullscreen, Web Workers,\n' +
        'CSS Animations, SVG, Gamepad API, Share API.\n\n' +
        '### Design Guidelines\n' +
        'Use CSS variables and RESPECT the user\'s AIMEAT theme: their light/dark choice from the AIMEAT pill is saved in localStorage "aimeat-theme" ("light"|"dark"). Light is the default; dark lives under [data-theme="dark"]; set that attribute from the saved choice on load (OS preference as fallback, live-update on change):\n' +
        '```css\n' +
        ...
        'Mobile-first responsive. Single HTML file with embedded CSS and JS.\n\n' +
        '### Important Rules\n' +
        '- Return the COMPLETE HTML file, not fragments\n' +
        '- Never use literal closing script tags in JS comments or strings (breaks HTML parser)\n' +
        '- Keep it as a single self-contained HTML file\n' +
        '- Gate AI features on AIMEAT.ai.isAvailable(); never hardcode an API key\n' +
