...
(() => {
  ...
  var data = {
    ...
    async discover(prefix, opts) {
      ...
      const res = await authFetch2("/v1/memory/discover?" + params.toString());
      if (!res.ok) throw new Error(res.error?.message || "Failed to discover public memory");
      const found = (res.data?.items || []).filter(function(i) {
        return i && i.key && i.owner_gaii;
      });
      const rows = [];
      ...
      if (o.includeMine !== false) {
        ...
        const mineRes = await authFetch2("/v1/memory?" + own.toString());
      ...
      const theirs = found.filter(function(i) {
        return !seen.has(i.owner_gaii + "\n" + i.key);
      });
      if (!withValues) return rows.concat(theirs.map(function(i) {
        return discoverRow(i, false);
      }));
      const read = await mapAtMost(theirs, DISCOVER_READS_AT_ONCE, function(item) {
        return data.getPublic(item.owner_gaii, item.key).then(
          function(value) {
            return { item, value, error: null };
          },
          function(error) {
            return { item, value: null, error };
          }
        );
      });
      const failures = read.filter(function(r) {
        return r.error;
      });
