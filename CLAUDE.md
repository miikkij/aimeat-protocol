...
- **Pre-commit hook** (`.githooks/pre-commit`) runs 41 commands, and the hook's own header comment is the current list: lint, four type checks, the surface and catalogue checks, the language and licence checks, then the audit ratchets and the vitest suite. Naming them twice is how the count in this line drifted to "twenty-one" while the hook grew past thirty, so `pnpm check:doc-counts` now holds this number against what the hook actually runs — add a check and this line fails until it is bumped, which is the point. `check:mcp-tools` proves the MCP surfaces expose the same tool NAMES and `check:mcp-schemas` proves they take the same PARAMETERS; the second was written in May 2026, left unwired, and a crew found the defect it had been printing all along. **Wire a new gate into the hook, not only into `check:audit-gates`** — that script is a CI job, so it answers "did this land on main" and not "may I commit this"; two gates sat in it alone for a day because the difference is easy to miss.
- **A ratchet is seeded, and the seed says what it is.** `security/*.json` and the `ALLOWED` map at the top of a gate script hold today's offenders with a reason each, so the gate fails only on a NEW one and the backlog is countable. Three of them are honest about being untriaged (`identity-resolution` at 141 units, `route-scopes` at 347, dependency-cruiser's 335): an entry there is a question nobody has answered, not a clearance, and triaging one means deleting it or replacing the sentence with what makes the code right. Re-seeding forgives the whole backlog, so it is a decision to be asked about, never maintenance.
