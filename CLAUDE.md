...
- **The guard tier blocks a merge; the full sweep does not.** `pnpm test:e2e:guards:sqlite` and `pnpm test:e2e:guards:postgres-kysely` run the 64 suites CI refuses to merge without, every one of them a refusal or an isolation boundary; the runner prints the assertion total (2226 on 2026-09-04) and a few minutes per backend. **A suite joins by earning it, never by looking safe:** alone, on a freshly deleted database, three consecutive identical green runs on BOTH backends. The 44 promoted on 2026-09-04 were measured that way — 264 runs, zero flakes, and per-suite counts identical between the backends. Run it before pushing anything under `src/routes/`, `src/auth/`, `src/services/` or `src/storage/`. Both E2E steps were `continue-on-error: true` until 2026-08-15, so no red suite had ever stopped anything; the full sweep stays advisory because it takes two hours and §18 makes it occasionally wrong. Fixing a suite so it can join the tier is the intended direction. → `docs/coding-guidelines/testing-requirements.md` Rule 1b
...
- **Pre-commit hook** (`.githooks/pre-commit`) runs 44 commands, and the hook's own header comment is the current list: lint, four type checks, the surface and catalogue checks, the language and licence checks, then the audit ratchets and the vitest suite. Naming them twice is how the count in this line drifted to "twenty-one" while the hook grew past thirty, so `pnpm check:doc-counts` now holds this number against what the hook actually runs — add a check and this line fails until it is bumped, which is the point. `check:mcp-tools` proves the MCP surfaces expose the same tool NAMES and `check:mcp-schemas` proves they take the same PARAMETERS; the second was written in May 2026, left unwired, and a crew found the defect it had been printing all along. **Wire a new gate into the hook, not only into `check:audit-gates`** — that script is a CI job, so it answers "did this land on main" and not "may I commit this"; two gates sat in it alone for a day because the difference is easy to miss.
