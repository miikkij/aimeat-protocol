...
- **Never inflect an English technical noun into Finnish.** The established Finnish word, or the English one as it stands; a third form is invented at the keyboard and reads that way. A test **suite** is a `testisarja`, never a "sviitti". Binds chat, commit messages and records written to the node as much as interface text, because no gate reads those.
