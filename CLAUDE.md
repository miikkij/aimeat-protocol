...
- **A ratchet is seeded, and the seed says what it is.** `security/*.json` and the `ALLOWED` map at the top of a gate script hold today's offenders with a reason each, so the gate fails only on a NEW one and the backlog is countable. `identity-resolution` and `owner-principal` are fully triaged: every entry says what makes that code right. `open-writes` (doors that write and take no credential) is triaged too, and its file says the result out loud: not one is a hole, because each is gated inside its handler by something a middleware scanner cannot see. `route-scopes` and dependency-cruiser's baseline are not triaged, and an entry there is a question nobody has answered. Triaging one means deleting it or replacing the sentence with what makes the code right. Re-seeding forgives the whole backlog, so it is a decision to be asked about, never maintenance. **The counts are not written here.** `pnpm debt` prints them with the date each last fell, and each file's own header says what its entries mean. This line used to carry five of them and by 2026-09-12 every one had drifted, three by more than half, including a claim that three owner-principal entries still said OPEN QUESTION when somebody had already answered all three.
