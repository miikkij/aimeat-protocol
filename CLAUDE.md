...
**Jouni's words are the criterion, quoted, never paraphrased.** A brief to a subagent quotes the requirement as he wrote it; when it reads two ways, or when two of his messages disagree, ask him before acting. **A change meant to keep the look (a refactor, a unification) passes only with no visible difference from the original page**, compared side by side on the same data; a difference stays only if he approved it by name beforehand. **The builder does not grade its own differences**: every difference goes on a list with before and after pairs for him or a separate checker, and a label such as "unification" or "standard look" accepts nothing. Overflow, console and gate counts are floors, not the verdict. **Visual work is shown on one page first**, on real data, before the rest starts, and stays in the worktree until he has seen it; for visual work "done" means he accepted it. Written 2026-09-23 after nine hours of a style unification were rolled back (`cee2cbaf5`): the brief had turned "make it look the same" into "consistent, not pixel-identical", twelve agents built to that, labelled their own differences as unification, and six commits reached `main` before he saw a page. → `docs/pitfalls.md` §94
