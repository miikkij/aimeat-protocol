...
- **Pre-commit hook** (`.githooks/pre-commit`) runs 38 commands. The hook's own header lists them in order and is the copy to read; naming them twice is how the count in this line drifted to "twenty-one" while the hook grew past thirty, and `pnpm check:doc-counts` now holds this number against what the hook actually runs. Add a check to the hook and this line fails until it is bumped, which is the point. Four of them are not `check:*` — lint, the three type checks — and the last is the vitest unit suite. `check:mcp-tools` proves the MCP surfaces expose the same tool NAMES and `check:mcp-schemas` proves they take the same PARAMETERS; the second was written in May 2026, left unwired, and a crew found the defect it had been printing all along.
