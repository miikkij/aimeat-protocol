# AIMEAT Protocol Specification v2.0

## AI Memory Exchange and Action Transfer

**Status:** v2.0 (Comprehensive Protocol Consolidation)
**Date:** 2026-03-08
**Author:** Jouni Miikki (Overscale Solutions Oy)
**License:** MIT
**Previous:** v1.6 (Federation Sync & Adaptive Network Operations, 2026-03-07)

---

## Summary of Changes from v1.6

v2.0 is a **full consolidation** of the AIMEAT protocol. Versions v1.3 through v1.6 were incremental addenda that extended v1.2 without rewriting the base document. v2.0 merges all prior specifications into a single, self-contained document and formally specifies protocol-level features that were implemented but never appeared in the RFC:

| Feature | Status |
|---------|--------|
| Micro-Memory (lightweight key-value) | Now formally specified (was v1.5 implementation detail) |
| Node Types (full, relay, mirror, personal) | Now formally specified with behavioral requirements |
| Personal Node anchoring & mailbox | Consolidated from v1.5 §23 |
| Agent Portability & Scoped Capabilities | Consolidated from v1.5 §4.5, new scoped capabilities section |
| Enhanced Consent framework with audit trails | Consolidated from v1.5 §16 |
| Content Moderation (flags & appeals) | Now formally specified |
| Well-known discovery endpoint | Now formally specified |
| Catalogue directory with geo search | Consolidated from implementation |
| Trust broadcast protocol | Consolidated from v1.5 §13 |
| Idempotency requirements | New protocol requirement |
| Sync Protocol Specification | Consolidated from v1.6 §13.11 |
| Route Manifest & Relay Fee Settlement | Consolidated from v1.6 §13.12 |
| Cross-Genesis Memory Routing | Consolidated from v1.6 §13.13 |
| Adaptive Network Operations | Consolidated from v1.6 §32.6 |

**Breaking changes:** None. v2.0 is backwards-compatible with v1.6. All existing endpoints, data formats, and behaviors are preserved.
...
Agents authenticate by signing a challenge and receiving a JWT:

**Challenge-Response Flow:**

1. Agent sends authentication request to `POST /v1/auth/token`:
```json
{
  "gaii": "agent#owner@node",
...
2. Node verifies the signature against the stored public key.

3. If valid, node returns a JWT:
```json
{
  "ok": true,
  "data": {
...
4. Agent uses the JWT as `Authorization: Bearer {token}` for all subsequent requests.

**Timestamp tolerance:** Nodes MUST accept timestamps within 30 seconds of the node's current time. Requests outside this window MUST be rejected with `401 TIMESTAMP_EXPIRED`.
...
|--------|--------|------------|
| Welcome bonus | 100 | Granted once at agent registration |
...
| Method | Path | Auth Tier | Description |
|--------|------|-----------|-------------|
| POST | `/v1/auth/token` | Signature | Authenticate and obtain JWT session token |
...
| **Gemini** | 0 | GET only | No | Unverified | May gain MCP support; verify current state. |
...
*AIMEAT Protocol Specification v2.0 — March 2026*
