...
2. **Policy (structured, machine-readable)** — a structured block holding per-capability **band** (`act`/`draft`/`ask`/`off`), budgets, the `stopSpending` flag, and active goal refs. **The scheduler reads this, never the prose.** Generated-as-a-proposal, **UI-confirmed** (it spends money / takes irreversible action, so it must not live in free text the LLM paraphrases).
   > **P4-A — built differently than this section first stated (decision: accept, 2026-06-24).** The policy block does **not** live on the agent-directives record; it lives **per-context in the owner-memory `secretary.config`** (`contexts[].policy = { stopSpending, dailyMorselBudget, bands }`, see §22). The scheduler reads it from there — `executeSecretaryJob` does `getMemory(owner,'secretary.config')`, picks the active context, and gates on `active.policy.stopSpending` / `dailyMorselBudget` / `bands` (`services/scheduler.ts`). `secretary.config` is the natural home because a Secretary has **N contexts** (one directives record can't carry N independent policies), and it co-locates the policy with the per-context brain/organism/history. The directives record keeps only the token-level `budget_limits`. No code change was made — the section is amended to match the implementation.

**Company secretary (P4-B — built as a 4th merge source, 2026-06-24):** the `ee/` module contributes a **locked directives layer** as a new **`source: 'enterprise'`** in the directives merge, **ranked above owner/agent and read-only**. Implemented as a **live overlay**, not a persisted copy: `routes/agent-directives.ts` GET, for an agent tagged `system:company-secretary`, resolves the org from its `org:<slug>` tag and calls the seam's `secretaryDirectives(org:<slug>@<node>)` (`services/secretary.ts` `resolveEnterpriseDirectiveLayer`), emitting those rules with `source:'enterprise'` + `locked:true` above the owner/agent layers; the response carries `enterprise_locked`. Because it is sourced live (never written to the agent's `rules`), the edition can **swap a company's brain** without a migration, **each company resolves its own** brain (multi-company), and the agent/owner layers below stay free for future per-owner rules **under** the locked layer. `ensureCompanySecretary` therefore no longer persists the brain; the merge is read-only because PUT only ever writes the agent layer. Community (the stub omits `secretaryDirectives`) yields an empty layer, so every other agent — and every Community node — is unaffected. The employee gets a read-only view to understand behavior; cannot edit either layer.
