Now I have all the content. Let me compile the complete extraction. Here is every template string from every function across all 7 files:

---

## FILE 1: foundry-prompts-base.js

---
### AIMEAT_CONTEXT (foundry-prompts-base.js:17)
```
...
---
### INSTRUCTION_DISCLAIMER (foundry-prompts-base.js:69)
```
...
---
### COMPONENT_TEMPLATES.csm (foundry-prompts-base.js:74)
```
${AIMEAT_CONTEXT}
...
---
### COMPONENT_TEMPLATES.msm (foundry-prompts-base.js:139)
```
${AIMEAT_CONTEXT}
...
---
### COMPONENT_TEMPLATES.extension (foundry-prompts-base.js:195)
```
${AIMEAT_CONTEXT}
...
---
### COMPONENT_TEMPLATES.app (foundry-prompts-base.js:468)

This is a function with dynamic construction. The returned template is:
...
The `cortexInstructions` variable (injected when `hasCortex` is true) contains:
...
${(() => {
  // ... generates per-cortex-lib sections with:
  // ### ${lib.label}
...
The `translationKeysSection` variable (injected when translation components exist) contains:
...
---
### COMPONENT_TEMPLATES.memory (foundry-prompts-base.js:878)
```
${AIMEAT_CONTEXT}
...
---
### COMPONENT_TEMPLATES.translation (foundry-prompts-base.js:927)
```
${AIMEAT_CONTEXT}
...
- Finnish translations must be natural Finnish with correct characters (a, o, a) — not machine-translated
...
---
### COMPONENT_TEMPLATES.cortex (foundry-prompts-base.js:996)

This is a very long template. The returned string is:
...
---
### NAMESPACE_RULES (foundry-prompts-base.js:1684)
```
...
---
### SANDBOX_CONSTRAINTS (foundry-prompts-base.js:1723)
```
...
---
### HTML_ENTITY_RULES (foundry-prompts-base.js:1872)
```
...
---
### EXTENSION_CONSUMPTION_RULES (foundry-prompts-base.js:1804)
```
...
---
### INIT_CONTRACT (foundry-prompts-base.js:1848)
```
## Init Contracts
...
## FILE 2: foundry-prompts-build.js

---
### buildBlueprintPrompt (foundry-prompts-build.js:59)
```
${INSTRUCTION_DISCLAIMER}${AIMEAT_CONTEXT}
...
[... followed by extensive rules sections for dataModel, Component Dependencies, Extension vs Cortex vs App Decision Framework, Cortex-Modular Architecture, Settings, and Test Scenarios as shown in lines 182-357]
```

(The full text of the rules sections from lines 182-357 was already shown above in the file read -- they are part of the same template literal.)

---
### buildInterviewPrompt (foundry-prompts-build.js:366)
```
${INSTRUCTION_DISCLAIMER}You are a requirements analyst for the AIMEAT service foundry.
...
2. COVER THESE AREAS (in order):
   a) USE CASES [... extensive interview protocol ...]
   b) DATA SOURCES [... with URL VALIDATION PROTOCOL ...]
   c) DATA MODEL [...]
   d) VIEWS & INTERACTIONS [...]
   e) STYLE & LOOK [...]
   f) SETTINGS & EXTERNAL SERVICES [...]
...
6. WHEN THE INTERVIEW IS COMPLETE:
   [... outputs JSON spec format ...]

Begin the interview now. Start by greeting the user and asking about their technical level.
```

(The full JSON output format from the interview is at lines 538-633.)

---
### buildExtensionSkeletonPrompt (foundry-prompts-build.js:1096)
```
...
---
### buildDataCortexSkeletonPrompt (foundry-prompts-build.js:1199)
```
${INSTRUCTION_DISCLAIMER}
...
---
### buildFeatureCortexSkeletonPrompt (foundry-prompts-build.js:1246)
```
${INSTRUCTION_DISCLAIMER}
...
---
### buildAppDomainCortexSkeletonPrompt (foundry-prompts-build.js:1313)
```
${INSTRUCTION_DISCLAIMER}
...
---
### buildAppSkeletonPrompt (foundry-prompts-build.js:1408)
```
${INSTRUCTION_DISCLAIMER}
...
---
### buildExtensionUnitPrompt (foundry-prompts-build.js:1467)
```
${INSTRUCTION_DISCLAIMER}
...
---
### buildCortexMethodUnitPrompt (foundry-prompts-build.js:1533)
```
${INSTRUCTION_DISCLAIMER}
...
The `accessPattern` variable when `unitDef.calls` exists:
```
...
The `accessPattern` variable when `unitDef.reads` exists:
```
## Memory Read Pattern
```javascript
...
---
### buildFeatureCortexSectionPrompt (foundry-prompts-build.js:1627)
```
${INSTRUCTION_DISCLAIMER}
...
---
### buildAppViewUnitPrompt (foundry-prompts-build.js:1764)
```
${INSTRUCTION_DISCLAIMER}
...
---
### buildExtensionAssemblyPrompt (foundry-prompts-build.js:1813)
```
${INSTRUCTION_DISCLAIMER}
...
---
### buildCortexAssemblyPrompt (foundry-prompts-build.js:1888)
```
${INSTRUCTION_DISCLAIMER}
...
---
### buildAppAssemblyPrompt (foundry-prompts-build.js:2011)
```
${INSTRUCTION_DISCLAIMER}
...
<body>
  <!-- Error collector script -->
  <script>[... error collector ...]</script>
  ...
  <script>
    ...
    (async function() {
      try {
        ...
        [... 5 platform UI cortex loads ...]

        // 3. Load domain cortex libraries
    ...
    async function startApp() {
      var cortex = AIMEAT['<camelCaseName>'];
      if (!cortex) { ... return; }
      await cortex.init();
...
1. **MUST start with `<!-- AIMEAT App Manifest ... -->` comment**
...
5. **MUST load cortex scripts in dependency order**
6. **startApp() is called ONLY after successful login**
7. **Do NOT modify the view unit implementations.**
8. **Include the error collector script** at the top of body.
...
## FILE 3: foundry-prompts-test.js

---
### buildTestPrompt (foundry-prompts-test.js:19)
```
${INSTRUCTION_DISCLAIMER}You are generating TEST CODE for a component in an AIMEAT service.
...
The `browserEnvDoc` and `serverEnvDoc` variables are embedded inline in the prompt (lines 131-307) -- their full text is shown in the file read above.

---
### buildExtensionTestFirstPrompt (foundry-prompts-test.js:359)
```
${INSTRUCTION_DISCLAIMER}
...
---
### buildTestFirstPrompt (foundry-prompts-test.js:426)
```
${INSTRUCTION_DISCLAIMER}
...
## FILE 4: foundry-prompts-cortex-data.js

---
### buildDataCortexPrompt (foundry-prompts-cortex-data.js:25)
```
${INSTRUCTION_DISCLAIMER}
...
1. **Extension runtime data** [...]:
   → Read with: `AIMEAT.data.getPublic('ext:EXTENSION_NAME', key)`

2. **Owner/user data** [...]:
   → Read with: `AIMEAT.data.get(key)`

[... full output format with YAML manifest and JS IIFE template including callExt, readExtMemory, readOwnerMemory, dv() helpers ...]
...
## FILE 5: foundry-prompts-cortex-feature.js

---
### PLATFORM_UI_EXAMPLES (foundry-prompts-cortex-feature.js:19)
```
...
[... Input, Select, Toggle examples ...]

### Dialogs (AIMEAT['aimeat-ui-dialogs'])
[... toast, Confirm, Modal examples ...]
```

---
### buildFeatureCortexPrompt (foundry-prompts-cortex-feature.js:141)
```
${INSTRUCTION_DISCLAIMER}
...
[... translation loading pattern with ${dataCortexBundle?.registeredAs || 'SERVICE_NAME'} ...]
...
[... YAML manifest template + JS IIFE template ...]
```

---

## FILE 6: foundry-prompts-cortex-app.js

---
### buildAppDomainCortexPrompt (foundry-prompts-cortex-app.js:23)
```
${INSTRUCTION_DISCLAIMER}
...
## Auth Pattern
[... code example with mountLoginButton ...]

## Translation Pattern
[... loadTranslations and t() code examples ...]
...
## FILE 7: foundry-prompts-fix.js

---
### buildBlueprintFixPrompt (foundry-prompts-fix.js:22)
```
${INSTRUCTION_DISCLAIMER}Your previous blueprint response was not valid. DO NOT try to fix the old response — generate a fresh one.
...
---
### buildExplainPrompt (foundry-prompts-fix.js:44)
```
You just generated a ${componentType} component. Before we validate it, explain what you built.
...
---
### buildReflectionPrompt (foundry-prompts-fix.js:60)
```
You are debugging a failed component. Your job is to DIAGNOSE the problem — do NOT write code.
...
---
### buildFixPrompt (foundry-prompts-fix.js:81)
```
${INSTRUCTION_DISCLAIMER}The following result had validation errors. Fix ONLY the errors listed below.
...
---
### buildFreshGenerationPrompt (foundry-prompts-fix.js:140)
```
${originalPrompt}
...
---
### buildImpactPrompt (foundry-prompts-fix.js:206)
```
${INSTRUCTION_DISCLAIMER}You are analyzing the impact of a change to an AIMEAT service.
...
---
### buildEditPrompt (foundry-prompts-fix.js:258)
```
${INSTRUCTION_DISCLAIMER}${AIMEAT_CONTEXT}
...
---
### buildFoundryReflectionPrompt (foundry-prompts-fix.js:326)
```
${INSTRUCTION_DISCLAIMER}
...
That covers every template string across all 7 files -- 7 shared constants (`AIMEAT_CONTEXT`, `INSTRUCTION_DISCLAIMER`, `SANDBOX_CONSTRAINTS`, `NAMESPACE_RULES`, `HTML_ENTITY_RULES`, `EXTENSION_CONSUMPTION_RULES`, `INIT_CONTRACT`), 6 `COMPONENT_TEMPLATES` entries (`csm`, `msm`, `extension`, `app`, `memory`, `translation`, `cortex`), and all prompt-building functions from the build, test, cortex-data, cortex-feature, cortex-app, and fix modules.