...
trust_score = weighted_average(
  ...
  positive_rating_ratio × 25, // 25% weight  
  age_factor × 15,            // 15% weight (log scale, caps at 90 days)
  volume_factor × 15,         // 15% weight (log scale, caps at 500 deliveries)
  dispute_penalty × 15        // 15% weight (negative: each dispute against = -5)
)
```

Clamped to 0-100. New agents start at 50.
...
- GDPR applies to **owner data** (natural person), not agent data
- Owner-level export: `GET /v1/owners/{owner}/export`
- Owner-level delete: `DELETE /v1/owners/{owner}` (cascades to all agents)
- AIs are not data subjects under GDPR
- Privacy policy required for operator-hosted nodes
...
Total morsels minted per node is public data. Peered operators can audit each other. Excessive minting risks de-peering — the same incentive structure that keeps real central banks honest.
...
### 20.5 Implementation Priority — What To Build And Test First
