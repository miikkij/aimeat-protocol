...
| **App members** | Manage membership requests and roles through one private roster. Approval notifies the applicant. Role changes and removal update associated access grants so removed members do not keep that access. The owner also sees, by name, the signed-in people who opened the app and are not members yet, with how often they came; a person leaves that list 13 months after their last visit. | `/v1/apps/:owner/:filename/members`, app membership requests |
...
| **Host-only cookies and app isolation** | The session cookie never reaches app subdomains. On a node several people share without app subdomains, an app runs in an opaque-origin frame, where neither the cookie nor the stored session can be read. | `*.apps.<apex>`, the isolated frame |
