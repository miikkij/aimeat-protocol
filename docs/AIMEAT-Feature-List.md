...
| **Workflow spending limit** | Cap what one run may spend on AI through its AI steps, in US dollars. Before the next AI step starts, a run whose AI steps have spent the limit stops, keeps what the finished steps produced and says how much was spent. Each step records its own AI cost. The daily AI budget still applies to every AI call. | `maxCostUsd` on `/v1/workflows/:id`, `aimeat_workflow_save` |
