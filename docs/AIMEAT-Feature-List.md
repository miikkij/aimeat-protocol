...
| **Design Book** | The shelf of every part, shown running: thirteen whole-page genres, starting shapes, looks and a Phaser page. An AI searches it, adopts a part, or proposes a new one, which a bench checks first. A search given nothing answers the whole shelf on one page, every part on a line under its kind, and the build specification carries the same page, so a builder sees what exists before it makes anything. | `design-book.apps.aimeat.io`, `/v1/designbook`, `aimeat_designbook_*` |
