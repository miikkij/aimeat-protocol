...
- **Run it before you push** anything touching `src/routes/`, `src/auth/`, `src/services/` or `src/storage/`. It is cheap enough that there is no argument for skipping it, and it is what CI will run. The pre-push hook (`.githooks/pre-push`) runs it for you when the commits about to go touch one of those four directories, both backends when the Postgres env file is present, and the whole unit suite; a push that touches nothing there skips the tier.
