...
- **Run it once, when the work is finished**, through `pnpm gate` (SQLite) or `pnpm gate:full` (both backends), before calling a change that touches `src/routes/`, `src/auth/`, `src/services/` or `src/storage/` done. Not on every commit and not on every push: a day of iterative work is twenty commits, and a tier on each of them is a day lost. The pre-push hook does not run it; CI runs it on every push and blocks a merge on it, so a push that skipped the gate is caught there, after the fact, without holding anyone up.
