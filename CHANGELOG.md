...
**Connect an agent by email, not just by handle — and one verified email now belongs to exactly one account, enforced at the database.** `POST /v1/agents/device-authorize` (the connector's `aimeat connect --owner …`) accepts the `owner` value as **either** the account handle (as before) **or** the account's **verified email** (case-insensitive): a value containing `@` is resolved to the owning account's handle before the RFC 8628 flow, so a new user who knows their email but not their handle can still connect. The email only *selects* which account the device request targets — it is never an auth factor (the human still approves the device code while signed in), so nothing about the flow's security changes. No match → `404 NO_ACCOUNT`; malformed → `400 INVALID_EMAIL`. The connector CLI is unchanged (it forwards `--owner` verbatim). Backing this, the one-verified-email-per-account-per-node invariant is now enforced by a partial-unique index on `emailHash` (both backends, dedupe-safe migration): `emailHash` became a strictly *verified*-email binding — `register-web` no longer stamps an unverified email at account creation (deferred to `verify-email`), and every verify/provision path refuses an email already verified elsewhere (`409 EMAIL_TAKEN`) ahead of the index. The email→owner resolver reads the full match set and asserts the invariant (logs + `409 AMBIGUOUS`) rather than silently picking.
