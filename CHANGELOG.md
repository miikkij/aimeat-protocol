...
**The owner's mailbox has two doors now, and neither of them is the one that was asked for.** A peer operator reported that an app holding `messages:read` got `403 Role owner required` on inbox, conversations and overview, because an app-grant token is always `role:'app'`. The requested fix, `requireRoleOrScope('owner','messages:read')`, was refused: on an agent, `messages:read` means the agent's own messages, and those routes fetch by the OWNER's name, so that gate would have let every `messages:read` agent read its sibling agents' conversations. An app reads the owner's mailbox on `messages:read`, and an agent reads it on `messages:read-as-owner`, a word the owner ticks on its own tick exactly as they do for `send-as-owner`. Four read doors, and what they show is the owner's own mailbox: not the agents' threads, not the agent list, not the groups. One service decides which of the three principal classes is asking and names the one word each needs in its refusal, so the 403 tells the reader what to tick.

**A door that refuses now leaves nothing of itself behind.** Read the ORDER of a handler, not just the presence of its check: a dozen doors did their work and then answered no. A refused broadcast had already written the message it would have sent; a refused passkey sign-in had already stamped the device; a refused `PATCH /v1/apps` had already written half the manifest; a refused workspace write left its embedded files opened; a package migration deleted the old component before asking whether the replacement would register; the cortex board ceiling was counted after the components were written. The memory delete guards moved into the service, so the MCP tool can no longer step around them. The MCP OAuth consent door reads the permissions the agent holds NOW rather than the ones it held when the flow started. The living-hooks allowlist judges the host the fetch will actually use, not the one in the string. An agent's own word about its risk can no longer lower the gate its owner set. A node keeps its seeded schemas after it is renamed. The package list door decides visibility and never the caller, and the upstream check stopped claiming it had verified a signature it never read.

**The node writes a date the way the person reading it writes dates.** Language decided the words and nothing decided the format, so a Finn reading English saw American dates and a clock nobody here uses. Two settings now carry that separately from the language, and `format.js` can tell a MOMENT from a CALENDAR SQUARE, which is the distinction that makes "today" and "2026-09-14" different questions. Relative phrases and durations follow the language rather than being assembled from words, because a word is not a unit. The served app libraries got one formatter instead of twelve, and `aimeat/no-raw-locale-format` refuses the thirteenth. Two scheduler defects fell out of the same work: a new schedule records the zone its hour belongs to, and one screen had been showing the same run as Monday 23:00 and Tuesday 05:00. Fifty-four strings nothing renders left the three language files, and the house they kept referring to does not exist.

**The operator's surface was rewritten page by page, and seventeen pages gave up a defect on the way.** This was not a repaint. The Packages summary read wrong at one of anything and its counts were capped at fifty; the MSM page was hiding ten of its rows and a public read leaked a variable name; the CSM list could never show anything at all; Knowledge emptied itself on a page number that was not a number, and three of its doors described the collection wrongly; the Push page said three things that were not true; the agent-integration and scheduler pages had nine numbers wrong; Statistics had been counting requests as zero since it was written; the chart labels on Statistics and Usage sat below the contrast floor; the GHII list was built at a width production does not have. Federation now says what is waiting on the operator and stops calling our own request theirs. Organisation sign-in says whether anybody can actually sign in and names the step that is missing. The apps page says which of the four states an app is in, and deleting leaves the row rather than the hole. Usage says whose money each figure is and can finally ask about the key it cannot count. The maintenance page has a stop that cannot lock its own operator out.

**Sixty-six commits moved the design language into one file, and two gates now keep it there.** A shape of the poster face is a class in `poster.css`, and a view sheet composes it rather than redrawing the rule, the slab, the sun or the box. `check:poster-shapes` refuses the next copy. The site also has ONE dialog, built on the browser's own `<dialog>`, with its actions in the dialog's footer; `check:dialogs` refuses a hand-rolled overlay, backdrop or `role="dialog"` box. The app catalogue's dialogs joined it, and they start at their top, which the hand-rolled ones did not.

**Four gates were added and one ratchet fell.** `check:field-reach` is the one worth naming: it measures with CodeQL, on every push, whether a record field the REST route accepts is reachable from a tool, and it exists because nothing compared a tool against the route it fronts. Only the two MCP surfaces were compared against each other, so `aimeat_company_update` not declaring `organism_id` answered "done" while writing nothing, and a peer operator hit it following our own package instructions. Its test names that report as the measurement. `check:dialogs` and `check:poster-shapes` are above. The silent-catch rule can now see a catch that answers with a SUBSTITUTE value, which is a different shape from an empty one. And `check:copied-logic` fell from six pairs to three: the broadcast's AI-provenance stamp is written once, and the welcome bonus, which all three doors that create an account wrote out themselves, is one function they all call. A skill can now be measured with `claude plugin eval`, `aimeat-writing` first.

**A packaged app arrived saying nothing about where it puts a person's data.** The manifest's data-map field is a STAMP whose address is `apps.<id>.datamap`, and the installed copy's id belongs to the installer rather than the author, so a carried stamp would point at a record that does not exist on the new node. It was therefore left out entirely, and the map went with it. The map document travels now, and the installer writes it under the name this node gave the app and stamps the manifest from what it wrote. One function validates, writes and restamps, called both by the owner editing their own app's promises and by the installer. COMPANY BRAIN ships the first one. Two more of its defects went at the same time, both reported from outside: `put_source` silently turned every unknown source kind into `chat` and now refuses one by name while accepting the three kinds that were missing, and the workspace name model no longer builds an em-dash into a name.

**The home page says whose turn it is, and the message list has sections.** The unread count answered how many and never which, so a new block names the conversations waiting on an answer and renders nothing on a day when none are. The message list itself now has sections, an archive, and rules the owner and their AI control; a failed read of those rules is an error rather than defaults quietly written back over them. The front page is the message frame's page, on its own address, with a projector whose show is a file and a script that photographs it.

Underneath all of it: the served libraries stopped failing quietly where the app-builder pitfall list had been telling builders to work around them, and the node now says what it did on every door where the same list had caught it silent. Every app-builder pitfall carries the date it was last checked, each trap has one home, and the list has an index by symptom. `AIMEAT.ai.complete()` sends the pictures a vision request is about. An app's crew-defs survive the upload road. `onSession` is one handler for a restore and a sign-in, and `onLogin` keeps its contract. Three open code-scanning alerts were closed, the expiry job stopped POSTing a caller's URL unguarded, and nodemailer moved to 10.0.9, out of reach of CVE-2026-90776.

## [Unreleased before 3.15.0]

The entries below were written between 2026-08-12 and 2026-09-09 and were never cut into a release heading, although the versions they shipped in were published. They are kept as they were written.
...
**A workflow that declares input could only ever be run at its defaults, from every tool surface there is.** `POST /v1/workflows/:id/run` has read `vars` and `target` since the CRUD phase in June. None of the three tool doors sent either: not the node's MCP, not the connector's, not the `/local/call` dispatch a fleet daemon uses. So a person asking their own AI to run a workflow could start it and could not tell it what to work on, which makes a workflow with a question in it a constant. It was found from the outside, by a sibling project converting a four-step recipe and having to run it at its default value because there was nowhere to put the question. All three doors take both fields now; the node's MCP makes the same mode mapping the route makes and reports the mode that actually ran rather than the one asked for, because a sandbox caller told "full" goes looking for its output under the unprefixed key. The CLI param-forwarding probe covers the two new parameters on its own and named both as dropped before the fix, which is the third time that file has caught this exact class and the reason it measures instead of reading source.

**And two defects inside the `ai` step, both in the one shape that step exists for.** Several steps answering a question in parallel and one assembling their answers is what an `ai` step was added for in August; it is also the shape neither defect could survive. `input_keys` did not honour `run.keyPrefix`, which `result_to_key` beside it and `from_key` on the datapackage step always did — two fields with the same job disagreeing, so a sandbox run wrote prefixed keys and read the live ones. The producing steps still went green, because signal evaluation has been prefix-aware since it was written, and the assembling step answered confidently from an earlier live run's data with nothing anywhere to say it had. Green and wrong is the worst shape available, and it was reachable from the one run a person makes when they are trying something out. The second is smaller and the same family: `input_keys` sat outside the save-time undeclared-var check its three siblings were inside, so a typo in a var name left a literal `{name}` in the key, matched no record, and the step reported the miss into its own prompt and passed. That is the right behaviour at run time and the wrong place to learn about a typo. The unit test seeds BOTH the prefixed and the live key with different values and asks which one reached the prompt, because a test that seeded only the prefixed one would have passed against the defect the day the live key happened to be empty.

**The SEO layer was already good and none of it was configurable, so every node but aimeat.io lied about who it was.** `public/spa.html` carried the identity as literals: a title, an og:image at aimeat.io, and two JSON-LD blocks naming AIMEAT and `overscalesolutions.com` in `sameAs`. `serveSpa` rewrote the HOST in those strings and nothing else, so a second node served structured data asserting it WAS that company — to Google, to every unfurler, and to every agent resolving "who is this". Two more failures rode along in the same file. `og:image` and the twitter card sat in the shell where `injectPageHead` never reached them, so all ten registry pages shared one social card and one Twitter blurb that disagreed with their own `og:` tags. And a Search Console verification tag could not be added without editing source, which is the first thing an operator has to do and was the one door shut to them. `injectSiteHead()` now stamps name, description, social image, verification tags and the two site-level JSON-LD blocks from config on EVERY served document — unconditionally, because that includes the routes with no registry entry, and `/v1/portfolio/:username` is served through there with no `routePath` and therefore had no head metadata of any kind. `injectPageHead` runs after it, so a page that describes itself still wins, and it now carries the twitter pair too: without that, one link unfurled with the page's own name everywhere except X. `seo.indexing` is the master switch a development or personal node needed and never had: off, it serves a Disallow-all robots.txt with no Sitemap lines and stamps `X-Robots-Tag: noindex, nofollow` on every response. Both halves are load-bearing and neither is enough alone — robots.txt stops the CRAWL, the header stops the LISTING, and a URL somebody else links to is indexed from the link text alone when only robots.txt refuses, because a crawler told not to fetch the page cannot read the noindex inside it. There was no `X-Robots-Tag` anywhere in this codebase before now. One default changes for aimeat.io and needs an env var in prod: `AIMEAT_SEO_SAME_AS` defaults to the project repository alone, because that is the only link true of every node, and the operator's own company is now theirs to set.

**Publishing an app was also, silently, a decision to have it indexed.** Every free, uncoded app went into `sitemap-index.xml` and served `User-agent: * / Allow: /` on its own origin from the moment it existed, and its owner had two states to choose between: parked, which removes the app from every surface, or fully public and indexed. Nothing in between, and no per-app brake for an operator whose domain was being used to farm keywords. Three parties have a say now, in this order. The EXISTING gate is unchanged: a priced or access-coded app was never eligible. Then the OPERATOR, who can block one app outright and who chooses through `apps.seo_mode` whether they are also the one who approves. Then the OWNER, whose switch is the whole decision in `owner` mode and a request in `review` mode. Default off, and that is the point: a published app is public and shareable by link immediately, and reach is a separate choice its owner makes on purpose. `services/app-seo.ts` holds the whole answer — four surfaces ask it (the sitemap index, the app origin's robots.txt, the `X-Robots-Tag` on the served document, the head metadata) and none of them recomputes it, because four copies of one rule is the exact shape that has already cost this project the same defect three times inside one MCP tool. The operator's block is a COLUMN rather than a manifest field, and that is the whole reason it works: the owner writes the manifest and rewrites it on every publish, so a block living there is one the blocked party lifts by pressing publish again. It is narrower than `operatorHidden` on purpose — a blocked app stays published, listed, usable and shareable, and only stops being findable. Two carry-forward decisions, both load-bearing: a REPUBLISH keeps the switch, because no publish door accepts the field and a manifest rebuilt without it would switch an app back out of search every time its author shipped; a FORK does not, because the manifest spread would otherwise hand a new fork the source's index flag and, in review mode, the operator's approval of a different app.

**Two pre-existing bugs the tests found.** `/sitemap.xml` had no `apexOnly` guard, so `https://apps.aimeat.io/sitemap.xml` answers today with the APEX sitemap, every `<loc>` naming a different host — a cross-host sitemap no robots.txt references, both wrong and useless. It also swallowed the app origin's own refusal, which is how the e2e caught it. And `app-head-meta.ts` stamped the literal `lang="en"` on every app whose author omitted the attribute, on a corpus that is substantially Finnish; it reads the app's own `aimeat-locales` meta now, with `en` as the last resort it always was. The cheapest win came along at the same time: every published app can have a screenshot and no app had a social card, while the image sat one unauthenticated route away. A search-visible app now carries `og:image`, a `summary_large_image` card, keywords from its tags, its declared language and its open count in the JSON-LD, and the screenshot lookup costs one owner-scoped metadata query only for an app that is actually search-visible.

**IndexNow moved out of the hand-run script, and the capability reaches an agent on all three doors.** `scripts/indexnow.ts` was the only way to send anything, over the static page registry, so no published app had ever been submitted — which is most of what changes on this node. `services/indexnow.ts` is the one submission now, called by the script, the publish path and the switch, and only on the way IN: there is no way to say "stop indexing" through IndexNow, and the robots.txt and noindex header the same switch flipped are what withdraw an app. `aimeat_app_seo_set` and `aimeat_seo_status` exist on the node MCP, the connector MCP and the CLI dispatch; `check:mcp-tools` caught the omission the moment the first two were wired, which is what it is for. Neither tool does the work — the status calls the same `buildSeoStatus()` the route renders, the switch calls the same `applyOwnerSeoUpdate()` the HTTP PATCH now calls, so the operator-field stripping, the merge, the note and the announcement happen where they were written once. Two gates earned their keep here: the status tool first called `buildSeoStatus()` directly, bypassing the `requireRole('operator')` on the route (a permission word is enforced on every door or it does not exist), and `no-storage-in-mcp` then refused the tool's own lookup, which moved into `ownerAppSeo()`. Deliberately NOT built, and a pre-existing gap rather than an omission of this work: there is still no MCP door for an app's other settings — `parked`, `forkable`, `access_code`, `protection` and the display name are HTTP-only on `PATCH /v1/apps/:filename`, so an agent cannot park its owner's app.

**The operator had no way to ask "am I findable".** The settings that decide it sat among two hundred others in the Config tab as raw dot-paths, and the only way to know whether a verification tag was reaching the page was to view source. The Discovery tab answers it in four parts, and reads from what is BEING SERVED rather than the configuration wherever the two could differ — the crawl policy as the document renders it (an empty content signal pairs itself to the training decision, so the raw field reports "unset" on a node that is serving a directive), the live counts, and how many app hosts the sitemap index will actually list. The steps are CHECKED rather than advised: steps 1 and 3 fetch this node's own front page and go green only when the tag is genuinely there, and the two that end inside somebody else's console carry a neutral dot, because claiming a step failed when we have no way of knowing is worse than saying nothing. The owner's half is a Search section in the app-catalog detail view, off by default, with a state line that says where the app actually stands rather than what the switch says — those differ on a review-mode node, on a blocked app (whose operator reason is shown to the owner, because a block they cannot read is one they cannot fix), and on a gated app where the switch is not offered at all. `/v1/pricing` and `/v1/app-store` joined the public-page registry: both answer the question a stranger arrives with, both are linked from the header, and neither was in any sitemap or had a canonical.

**Three things the browser found that no gate would have.** `/v1/admin/apps` did not carry `seo_state`, so every row in the operator's table rendered the raw key `DASHBOARD.SEO.STATE_UNDEFINED` — and that listing is the one the tab must read, because it is the only one carrying every app including the parked and hidden ones. The live-page check sliced the first 8 kB of the document, and the injected tags go in just BEFORE `</head>`, at the end of a head that is already twenty-odd kilobytes of importmap, so it reported a working tag as missing; it cuts at `</head>` now. And the table wrapper class had no CSS at all, so on a phone the columns were cut off at the viewport edge rather than scrolling. Verified at 1440, 768 and 390 with no page overflow and no console errors, the block round-tripping through the real UI, and the Finnish strings rendering rather than falling back.

Proven rather than asserted. Guards 501/501 on sqlite and postgres-kysely (migration `0051` applies on boot). `e2e-app-origin` 64/64 with a new phase asserting all four crawler-facing surfaces on both sides of every flip, that a blocked app still serves its users, that an owner cannot lift a block by flipping their own switch, that a second owner gets 404 on the owner door and 403 on both operator doors, and that the block survives them trying. `e2e-agent-readiness` 46/46, which pins the sitemap contents and is why it is the suite that would catch a bad registry entry; it gained a denial case of its own, because a suite that only proves things are reachable cannot tell an open door from a working one. New unit suites `app-seo` (16) and `page-head-site` (11), the second confirmed to go red when the implementation is broken in three separate places. The serve-marks goldens were re-captured for the third intentional output change — nine went red because the head-meta pass now stamps a robots meta on an app nobody asked to have found, which every existing fixture is; the assertion was right and the behaviour changed under it, and two cases were added for the search-visible state, which no golden covered.

Four files passed the 800-line ceiling on the way, so four pure moves rode along: `serveSpa` and the public-file locator to `routes/portal-spa.ts`, the knowledge memory-link methods out of the SQLite apps file, the commerce block out of `tool-call-defs-apps.ts`, and extensions plus cortex out of the organisms/workspaces/apps catalog slice. None of the four was ever a concern of the file it left; they lived there because the original extractions cut by file size rather than by subject.

Still open and deliberately not done here: portfolios and published knowledge packages carry no per-item switch yet, so they follow the node-wide setting rather than their owner's. Both are the same shape as the app switch and reuse its mechanisms, which is why the plan puts them after this ships.

**Sharing groups were a management API attached to nothing, and the tests said so if you read them.** `e2e-sharing-groups.ts` had 17 assertions covering create, list, add member, change permissions, remove and delete, and not one that read or wrote a memory record. `e2e-mcp-consent.ts` had 8 and never called `/v1/memory`. So both features had a fully tested management surface and no test that they affected access at all, which is the class `e2e-mcp-cross-owner.ts` had named in its own header two days earlier. A sibling project measured cross-owner reads on production, got 403 everywhere, and concluded the platform could not do it. Measured here on both backends before anything was touched, four defects, each of which answered `200` while doing nothing: SQLite never wrote the `groupId` column on any memory path (not the INSERT, not either UPDATE, not the compare-and-swap), so every `visibility:'group'` record on a SQLite node was unreadable by every member of the group it named, permanently, with the group's own name echoed back in the write response; neither backend wrote it on an UPDATE, so a record created `private` could never become group-visible afterwards, which is the ordinary sequence and almost certainly what the field report hit; moving a record between groups did nothing, the old audience keeping access and the new one never gaining it while the write reported success; and the cross-owner read door passed the raw JWT `sub`, which for an owner session is a bare account name rather than the GHII, so no membership and no consent grant could ever match it — a person could not read what had been shared with them while their own agents could, inverting the guess in the field report. The storage-file twin `GET /v1/pub` had fixed exactly that in July with a comment explaining why; this side of the pair was left. Diagnosis proven rather than argued: both candidate fixes were applied locally and the whole measurement re-run on both backends before anything was written for keeping. `storage/memory-sharing.ts` now holds the one rule about which group a record belongs to after a write, called by both providers, and it inherits — a writer that says `group` without naming one keeps the group the record already had, the same reasoning `trackable` uses a few lines away, so an agent appending today's output to a shared key cannot silently end the share by forgetting to resend an id.

**Then the record-field shape itself went, because it could not express what sharing is for.** `visibility:'group'` plus one `groupId` can name exactly one group, has to be repeated on every write, and cannot cover a key space that does not exist yet. The last is fatal for the only use anyone has: a subscription writes tomorrow's record tomorrow, and the reader must not need a second act from anybody to see it. A share is now its own row — `(owner, group, key pattern)` in `group_shares` / `GroupShare`, indexed both ways because the group direction is on every cross-owner read and the owner direction is the question ownership is judged by. The record keeps its own visibility, normally `private`: visibility is the floor and a share is a named exception on top, so sharing something never changes what it is, and one record can sit in several shares. The wildcards are the consent ones on purpose (`*` one segment, `**` the subtree) — a node where one wildcard means two things depending on which feature reads it is a node where nobody can predict what they agreed to. Both list directions exist, and the reader's is the half whose absence made the feature unusable: without `GET /v1/shares/incoming` you had to be handed an owner identity and an exact key out of band. Making a share is its own permission, `share:manage`, which no wildcard carries, and two things fall out of that: the gate cannot be a role check, since an agent's token carries its owner's `owner` role (`routes/auth.ts` copies it on) so `roles.includes('owner')` passes every agent of every owner — the same pattern still sits in `services/consent-write.ts`, where it means an agent can grant consent without `consent:manage`, left as a separate finding; and a pattern with no literal prefix, or one reaching a reserved server key, is refused rather than trimmed, because a share that quietly covers more than the person believed is worse than an error.

Existing `visibility:'group'` records keep working. `scripts/migrate-group-shares.ts` converts them, one share per record at that record's exact key, and is deliberately not a boot step: it creates access rights, and access rights should not appear because a process restarted. Dry run by default, idempotent, and it never widens anything. The surfaces followed the model: the Access tab lists what each group reaches with add and revoke plus a **Shared with you** section, the Memory tab badges a row a share covers and can share the key's own space pre-filled one level up, and `group` left every visibility picker. One defect there was found by the browser and not by reasoning — the shares list hung off `loadGroups`, which a seeded page never calls, so every group rendered as sharing nothing, which is worse than rendering nothing because it reads as an answer.

Proven rather than asserted. `e2e-key-space-shares` (29) and `e2e-group-sharing` (20) green on both supported backends, alongside sharing-groups, mcp-consent, memory-namespaces, memory-full, storage-visibility, mcp-memory-extended, workspace-member-records, workspace-public-records, organism-workspace-access, memory-file-presigned, mcp-cross-owner and app-grants. The app-grant path is measured rather than assumed, because the subscription flow's shape depends on it: a token with `share:manage` creates a share and one without it gets 403, and it may not create a group or admit a member — an app can hand out access to a key space but cannot assemble its own audience. The migration was run end to end on a seeded legacy record. The UI was driven in a real browser at 390x844, 1280x900 and 1280x460 (horizontal overflow 0 at each), 0 repaints on an open panel over 20 idle seconds, one request over 60. Two loose ends worth naming: `share:manage` existed on the node and in the app-grant vocabulary but had no row in the agent permissions editor, so an owner could not tick it for an agent at all (the round-trip test compares the frontend list against the frontend domains, so a scope missing from both sides passes); and `.ld-log-body` had no `white-space` rule, so every blank line in a public change log entry collapsed and a four-paragraph entry rendered as one wall.


## [2.8.0] - 2026-08-12

**The account's own controls stopped answering to anything acting in the owner's name, and the mint that made that possible stopped handing out roles.** Not one authenticated route under `/v1/ghii` or `/v1/ghii/totp` carried a role gate, and neither did account deletion or the full export; every handler keys off `req.auth!.owner`, which carries the human's account name on app-grant tokens, GEAI tokens, agent JWTs and agent PATs alike. Three chains were verified end to end rather than reasoned about. An OAuth-only account could be taken over with no proof of anything, because the current-password check is gated on there BEING a password. A password account fell in two calls: the same principal that repoints `notificationEmail` owns the mailbox the unauthenticated reset rail then mails the code to, so H-5 had closed the shortcut and left the loop. And two-factor could be armed with a secret and backup codes the human never sees, which locks them out permanently, because removing it needs a valid code and there is no operator reset. `requireOwnerPrincipal()` now gates those fourteen doors: the test `requireScope` already used for its owner bypass, extended to exclude `role:'app'`. An owner-level PAT passes, deliberately, because it is a credential the human minted for themselves behind `requireRole('owner')`; the tempting stronger test, "has a sessionId", would have signed real humans out, since the PAT-backed browser refresh mints owner tokens without one.

The companion word `account:security` exists so an agent CAN be given these doors on purpose. Nobody is grandfathered into it and no wildcard carries it, which is the point: this platform is agent-first and "humans only" would have been the lazy answer to a question the developer answered the other way.

Underneath it, `POST /v1/auth/token` was copying the owner's `owner` and `operator` roles onto the agent's JWT. The report called that a legacy path; `data/builtin-skills.ts` makes it the last step of the PAVED device-authorization path, so modern agents came through it. The sharpest consequence was laundering rather than the role test: that token passed `requireRole('owner')` at the PAT mint, `isOperator` was true so `grant_operator` was permitted, and the resulting PAT resolved to `['owner','operator']` with `scopes: []` and no `agent` role, at which point `requireScope` bypasses entirely. A scope-limited agent minted itself an unscoped operator credential in two calls. The mirroring is gone, along with the identical three lines on two unauthenticated mints in `web-verify.ts` and the self-heal branch that was the only mint path writing to the owner record. Four gates that trusted a bare `roles.includes('owner')` now exclude agents: `memory-write`, `consent-write`, and `ai.ts` plus `librarian.ts`, the last two of which let a mirrored agent spend the owner's AI budget without the `ai:use` word written to gate it.

**Five more holes of the same family, each closed where it was rather than where it showed.** `finance.` and `commerce.` join the reserved owner prefixes: `finance.accountants` is the cross-owner authorization list for the entire finance area and `commerce.psp` holds the seller's payout address and Stripe secret, and an app with `memory:write` could append itself to either. That refusal breaks one live app on purpose, and the app was fixed rather than the rule. A seller's app-tool could name a capability it does not own, and the internal pass carried a product coordinate the paywall never read, so a pass minted for one product excused payment on any other; grepping the paywall for a single use of that coordinate returning nothing was the whole proof. `escapeHtml` in the app catalogue serialised a text node, which escapes `& < >` and not quotes, while `filterAttr` built a double-quoted attribute out of another owner's manifest name, and `jsArg` one function away escapes quotes for exactly this reason, so it was an omission rather than a decision. Every stored-file download now sets `nosniff`, a `Content-Disposition` that is inline only for formats that cannot script, and a file-only CSP. Push subscriptions are keyed on `(ownerName, endpoint)` in both providers, so a second device joins the first instead of evicting it, and the three mutating routes require `push:manage`.

**Organism membership was granted carefully and revoked incompletely.** Removing, banning or leaving pruned `members[]` and `admins[]` and never `organism.agentGaiis`, and every membership gate treats a listed GAII as a member in its own right, so an ejected person kept full access through their own agent token and their workspace-role consents survived too. The code invite had the mirror: plain membership authorized it, and it then applied an attacker-chosen `org_role` and an attacker-chosen list of per-workspace grants with no further check, where its email twin requires creator or admin and re-authorizes each grant.

**Origin markers are not a client's to set.** `middleware/subdomain.ts` took `req.appOrigin` from a request header with no verification. The apex nginx was not blanking those headers in production and `AIMEAT_APP_ORIGIN_ENABLED` is `true` there, so one request header made the node believe a request had arrived on the isolated app origin, which defeats the whole app-origin isolation. That was fixed at the proxy the same day; the code now honours a marker only when the `Host` belongs to that family, because configuration drifts and this is the drift that shows up nowhere. The unit test for it had been passing a Host no family matched, which hid a second bug: `labelFromHost` fell through to the apex for the bare app host, so `apps.aimeat.io` described itself as the subdomain "apps".

**Two defects the test run exposed, neither of them a test problem.** An `undefined` in a patch object is a 500 on one backend and a silent success on the other: the sqlite provider spreads a patch over the whole row so `undefined` reaches the database as NULL and reads as "cleared", while the Kysely provider builds a partial `SET` and throws. `PUT /v1/ghii/cors` cleared the origin list exactly that way, so the setting worked on the development backend and 500'd on the production one, and the new `account:security` test is what finally drove it on both. And `bulkSetMemory` and `bulkDeleteMemory` ran `BEGIN` unconditionally, so calling either inside a caller's transaction threw and publishing a batch of workspace drafts answered 500.

Measured rather than asserted, and the measuring is half the story. The full sweep ran on both backends. Its total is not a regression signal on this runner: the database is cleared BETWEEN suites and not before the first one, and the server is killed on a fixed one-second wait, so a slipped restart hands the next suite the previous one's data and produces hundreds of 403s in suites that have nothing to do with the change. `e2e-packages` failed 78 times in one sweep and passes 95 of 95 alone against a fresh database, on both trees. Every failing suite was therefore re-run alone, on this tree and on a copy of the tree from before this work: **12 assertions broken by this batch, 77 cleared by it**, and every one of the 12 fixed. Eight of the twelve were stale setup rather than a changed rule, four asserted the hole. `e2e-companies` is the sharpest: it DID pass `Host` in a fetch headers bag, and undici drops Host as a forbidden header without a word, so that setup had never once reached the wire. One more thing worth knowing about this runner: a developer's own gitignored `aimeat/.env` is loaded by the test server, so two machines cannot compare results while it turns Google sign-in on for one of them.

Found and deliberately not fixed, because each is a decision rather than a repair: the capability webhook allowlist is skipped by omitting the `source` field, since the gate reads what the caller sent while the builder defaults a missing type to `manual`; `updateCapability` strips only `id`, `ownerGhii` and `createdAt`, so an owner can PUT `trust.operatorReviewed` and have the directory show their own capability as reviewed by the operator; and `requireRoleOrScope('agent', 'organism:write')` admits any principal holding the `agent` role BEFORE it looks at scopes, so the same agent that cannot SEE `aimeat_organism_create` gets `201 Created` from `POST /v1/organisms`.


**Asking for help stopped requiring an identity lookup, and the group conversation it needed was built rather than faked.** A GHII names one person on one node, which is correct and unusable as a support address: reaching an operator meant working out who ran the node and what their identity string was. Nobody does that. The last external report arrived six days late, by email, relayed by a human who happened to have a contact. `support@operators` (long form `support@{nodeId}`) now resolves at send time to every owner holding the operator role and opens one thread all of them see, in Messages, where they already answer people. Both `support` and `operators` are reserved names, so neither half of the address can be registered out from under it. `POST /v1/messages` and `aimeat_dm_send` share one resolver, so the two doors cannot drift; the response carries `conversation_id`, `participants` and `delivered_to`, and the follow-up goes back through the same `conversation_id`. A first-time sender is not held behind the first-contact gate, which is the whole point: someone asking for help must not land in a request queue nobody opens. A node with no operator answers `503 NO_OPERATORS` rather than accepting a message with no recipient.

Underneath it is a genuine n-party thread. Until now a conversation was a pair, its id a hash of the two sorted identities so both nodes derive it with no coordination and no stored row. That is right for two and cannot express three, so a group thread gets a `Conversation` record (SQLite table plus migration `0031_group_conversations.sql`) and a pair thread still stores nothing: the **absence** of a record is what says which one you are looking at, which is why this migrated no existing conversation. Membership is a JSONB array matched by containment on Postgres and by a quoted LIKE on SQLite, so `alice@node` can never match `alice@node-2`. Delivery stays per person: every participant holds their own mailbox copy, so read state, deletion, notifications and the agent connect-tunnel wake are the paths already written and tested, applied to n people rather than replaced. Mailboxes are de-duplicated before writing, because a thread naming both `alice@node` and `bot#alice@node` resolves to one mailbox and two rows would collide on `(id, ownerGhii)` halfway through a delivered send. `recipientGhii` on a group copy is the **thread's** address rather than a participant, which is a deliberate departure: in a pair that field answers "who is the other one", and in a group there is no such person, so naming the thread keeps it meaningful and makes every participant's conversation list agree. Stated rather than discovered: a group is node-local, and a cross-node participant is refused by name instead of half-delivered.

**The Node Feedback Channel is gone, table and all.** Reading the seven threads on production first is what decided it: they are better than most human bug reports, carrying schedule ids, UTC time windows, repro steps and suggested fixes. Not one had ever been answered or even triaged, because answering meant remembering that a second inbox existed behind an admin tab. Two of them had been fixed weeks earlier and neither reporter was ever told. That is the argument for Messages, and keeping a parallel channel would have rebuilt the same failure. Out with it: the MCP tools and their scope entries, the REST routes, the service, the repository, both providers' methods, the admin tab, its 60 locale keys, six OpenAPI paths plus the schema and tag, the SQLite table with its indexes, and `DROP TABLE "Feedback"` in `0033_drop_feedback.sql`. The connector's tool surface loses two entries with it, which is what `check:liaison-surface` caught and why this is 2.8.0; the npm package still needs publishing and its baseline re-recording.

**An AI living in the user's own tool stopped being handed a checklist it cannot finish.** A conversation-triggered agent (Claude Desktop, VS Code, any MCP client) registers in the default `interactive` mode and therefore got the full 13-step Hello Integration, one step of which asks for a webhook or a `lastSeen` inside ten minutes. It has neither and never will, so it sat at 9 of 12 forever while being fully connected, and its own attempt to escape made things worse because it guessed at the mode vocabulary (`"MCP"`) instead of the value the node accepts. `WORKSTATION_STEP_IDS` had the right four-step flow all along; nothing ever put such an agent into it. `identify_platform` is step two and the platform string already says whether an agent is node-resident, so the inference happens there, before the checklist has anything to strand on. Only an unchosen mode is replaced, so a deliberately set `task-runner`, `autonomous` or `coordinator` is untouched, and the response carries `mode_set_to` plus an explanation, because a step count that drops from 13 to 4 with no reason reads as lost progress. This was a real external user and it cost them a week.

**Which model wrote a message now reaches the reader, and closing that gap exposed a bigger one.** The model was already on the agent record (self-reported at `identify_platform`) and the provenance generator already had a field for it; nothing carried it across, so every stamped write said "an AI wrote this" and nothing about which. `stampAgentWrite` now fills it, the inbox and thread reads resolve a page of records in one batched query, and the bubble shows it as the agent's own claim with a tooltip saying the node cannot verify it and that a platform may hand work to a cheaper model mid-session. Verifying that in a browser is what found the real defect: `POST /v1/messages` was not stamping provenance **at all** while `aimeat_dm_send` was, so the same act was recorded on one surface and unstated on the other. Fixed at the route, with the human sender left alone, because stamping a person's own words would be the one false statement this subsystem exists to prevent.

**The address is where a caller is stuck rather than in documentation they have already left.** `error()` carries it as a next action on every failure this node returns, which is one edit instead of thirty handlers; the MCP `instructions` string names it on connect; the agent handbook replaces its feedback section with it; `GET /` advertises it; and `GET /onboarding` hands it over in `hints.stuck` the moment a step has failed, which is precisely where the external agent invented three remedies of its own.

Proven rather than asserted. Six new E2E cases pin the address end to end on **both** backends: a stranger writes without knowing any operator identity, the operator finds it in the ordinary inbox with the sender visible, no first-contact hold, the reply lands in the sender's thread, the long form resolves the same operators, and an outsider posting into a thread they are not in gets `403 NOT_A_PARTICIPANT`. Four more pin the mode inference, including that an owner-chosen mode survives. Suites green on both providers (messages 30/30, agent-onboarding 69/69, agent-dm 15/15, mcp 41/41, plus broadcast, interactive and provenance). Driven in a real browser at 1280 and 390: the model badge renders, the support threads open as ordinary conversations, and there is no horizontal overflow at either width. That run also caught a bug of its own, which is the argument for doing it: the badge's tooltip used `{{model}}`, the server-side placeholder form, and rendered literally as `{claude-opus-5}` until it was changed to the `{model}` the SPA's `t()` reads. Three failures elsewhere were confirmed independent of this work: an unconfigured email service, an app-label viewport assertion, and an `AIMEAT_AI_COP_SECTIONS` mismatch in the Postgres test environment.

Still open and deliberately not done here: the seven feedback threads live in production until this ships and `0033` runs, and five of them are open questions that will go with the table. They are recorded outside the database or they are lost.

## [2.3.0] - 2026-07-27

**An extension install that did not happen stopped reporting success, and the pattern behind it was measured and removed across the codebase.** A field report said the presigned-ZIP path of `aimeat_extension_install` answered a bare `500` on a first install and `200 success:true` on an update while leaving both the manifest and the scripts untouched. Production was not behind — that was checked by decoding the presigned token's `meta` rather than assumed — so it was live behaviour. Three separate faults, each of which taught the wrong lesson because the system answered with nothing. `handleExtensionUpload` ended its upsert with `storage.updateExtension(...) ?? existing` over a Postgres provider whose `updateExtension` ended in `catch { return null; }`, so any write error became the OLD record returned as proof of a new deployment; the MCP tool did worse, reporting the NEW version number over unchanged code. `(a.method as string).toUpperCase()` ran unchecked, so a mis-typed manifest field threw out of the parser and the upload router's catch-all turned it into `PROCESSING_FAILED` naming nothing — the usual trigger being a comma inside an unquoted YAML flow map, which starts a new entry rather than continuing prose. And three copies of the manifest→record builder had drifted: the ZIP copy dropped per-action pricing (`tollMorsels`, `commercial`, `payMoney`, ODPS) and the `type: secret` marker, so a priced EXCHANGE capability became a free one with its API key in plaintext purely by being installed as a ZIP. All three paths now call the one `buildExtensionRecordFromManifest`, which also type-checks every field it later reads as a string, so a broken manifest is a `400` naming the field and what the YAML actually produced. Also on that path: ownership was unenforceable for a GHII token subject (`parseGAII` returns null without a `#`, so `installedBy` recorded `'upload'` and the `403` check short-circuited away, letting any owner overwrite anyone's extension), secret config is encrypted as on the REST path, EXCHANGE listings are re-projected, and inline mode with `scripts` but no `manifest` is an error instead of a silent fall-through that returns an upload URL and drops the scripts.

**`aimeat/no-silent-catch` now keeps that class from coming back, and cleaning to satisfy it found three real bugs.** Measured with the rule rather than by eye, **1221 handlers** discarded their error: 700 empty, 297 returning an absence value, 267 never mentioning the error at all. The structural finding is why it survived so long — the postgres-kysely provider swallowed write errors into `null` while the SQLite provider threw, so a failed write on the *primary production backend* was indistinguishable from a missing row while local tests stayed green. `src/` (426 sites) and `public/` (591) are now at zero and enforced at `error`; `pnpm check:silent-catch` reports the backlog per area and `--strict` runs in the pre-commit hook and CI, failing only on areas already at zero so the gate protects what is clean without blocking the rest. The rule accepts any handler that *surfaces* the error — an HTTP response, a promise rejection, a typed failure carrying the message, a `setErr`-style UI setter — because a rule demanding literally log-or-throw would fill with disables and stop meaning anything; deliberate silence stays available with a reason on the line. Where an author had left a comment on a swallow, that comment became the log message: it states why the silence was thought safe, which is exactly what an operator needs when it was not.

**The password brute-force lockout had never worked, on either backend.** `config.passwordLockoutAttempts` was dead code in the login route. Postgres deleted `passwordFailedAttempts` and `passwordLockedUntil` before every update as "not columns" — true, while their TOTP equivalents existed — leaving a statement with nothing to set whose error was swallowed into `null`; SQLite had the columns but omitted them from its `INSERT`, both `UPDATE` paths and the row deserializer. Every wrong password therefore read back "0 attempts so far", and rate limiting was the only remaining brake on password guessing. Nothing noticed because the only assertion about a wrong password was that it returns `401`, which never stopped being true. Migration `0013_ghii_password_lockout.sql` adds both columns, both providers write and read them, an all-fields-filtered update is a no-op returning the current row instead of invalid SQL, and `e2e-auth-lib` now asserts the **lockout** — six wrong passwords must stop returning `401`, and a locked account must refuse the *correct* password — on its own throwaway account. Two further finds from the same pass: `auth/node-keys.ts` returned `null` for any key-file read error, and the caller's answer to `null` is to generate a NEW keypair, silently changing the node identity so issued JWTs stop verifying and peers that pinned the old public key reject it (unreadable, corrupt or undecryptable now refuses to start; a failed persist logs at error); and a failed workspace-manifest read was being cached as "this workspace has no manifest".

**The SPA records suppressed failures instead of printing them.** `public/js/swallowed.js` keeps a bounded ring buffer readable at `AIMEAT_SWALLOWED()` on any page, with console output opt-in via `?debug=1` or `localStorage.aimeat_debug`. Six hundred new `console.warn` calls would have destroyed a signal this project measures against — Rule 1b verification treats a clean console as evidence — so the default is quiet but never silent. Browser idioms where the throw *is* the answer (storage blocked, no user gesture, a cache entry that is not JSON) carry a disable saying so.

Proven rather than asserted, and three of the proofs are corrections. Nine unit tests pin the manifest builder's parity across the inline and ZIP paths; the rule ships with its own tests covering **both** directions, because a lint rule wrong either way is worse than none. `e2e-presigned-meta` no longer trusts the response it is testing: it ships a visibly different v2 script and proves the upsert by reading the server back and **invoking** the action, plus cross-owner `403`, malformed manifest and non-ZIP cases. Full sweeps green on **both** supported backends (3762 of 3768 on PostgreSQL+Kysely with a fresh schema, the same six failures being config-gated money and email paths, confirmed independent of this work by re-running the identical suites on a pre-change tree). The SPA was driven in a real browser: registration, seven profile tabs rendering, no horizontal overflow, no JS console errors, an empty buffer after navigation, and a deliberate probe recorded to prove the buffer works rather than merely reads empty. Along the way three mistakes were caught before they shipped and are worth recording: rewriting `.catch(() => null)` to `.catch(err => log(...))` silently turns `null` into `undefined`; a log message derived from nearby lines named the *wrong* function, and a log that lies costs more than one that is vague; and CRLF line endings mean `.` does not match `\r`, so every single-line pattern matched nothing while the tool reported success over files it had never touched — the exact bug class being fixed, inside the tool doing the fixing.

Still open and deliberately not gated: `src/static` (204 sites — the served SDK libs and the app-catalog bundle). They are browser code in a different style, `function ()` callbacks rather than arrows, and each bundle needs its own reporting channel, so they are their own piece of work rather than a variation on this one.

**A DM attachment is sent as bytes, and the ceiling that blocked a 10.6 MB video was one nobody could have found.** The attachment path base64-encoded the whole file into a JSON body. Base64 inflates by 4/3, so a 10.6 MB video became a ~14.8 MB request that had to fit under `security.json_body_limit_large_mb` — a limit that lives in a different section of the admin config from `quota.storage_max_file_size_mb`, the one an operator raises when a file is refused for being too large. Raising the storage limit to 15 MB therefore changed nothing, and nothing in the failure pointed at the ceiling that was actually binding. Attachments now take the presigned path: mint a one-shot URL, `PUT` the raw file. `/v1/upload/` skips body parsing entirely and streams, so the inflation and the JSON ceiling both stop applying and the only cap left is the storage limit the operator set. The `PUT` uses a bare `fetch` on purpose — the token is the capability so there is no auth header, it is single-use so a retry would answer `409`, and a large upload outlives `api()`'s 30 second timeout.

Two gates that only the inline path ran had to move with it, or the switch would have widened them. The account-wide storage quota is now enforced in `handleStorageUpload`: the token's `maxBytes` caps one file, the quota caps the account, and presigned was an unmetered way around it. The `anonymous/*` namespace fence now applies before the token is minted, where it sat after the presigned early return, so asking for an upload URL walked straight past it; the gate belongs on the write rather than on the representation the caller happens to choose.

Driving the browser to verify the fix found that the failure had been invisible. `inbox-tab` discarded `err.message` and toasted a generic "could not send", so an oversized video read as a dead button with nothing in the console either. Failed sends now carry the thrown reason (EN + FI), which is the only form in which "the file is 10.6 MB and this node accepts 10.0" reaches the person who can act on it. Proven rather than asserted: three new `e2e-anonymous` cases pin the fence in **both** representations, including that a refused mint hands back no `upload_url` and that an allowed key round-trips 7 raw bytes with no base64 inflation. Green on both supported backends (183 assertions on PostgreSQL+Kysely, 210 on SQLite across upload, presigned-meta, storage-visibility, attachment-sweep, anonymous, messages, agent-dm and file-handoff). In a real browser the 10.6 MB video went mint `200` → `PUT 200` → `POST /v1/messages 201`, and the stored file is 11,106,928 bytes carrying an intact MP4 `ftyp` header, which is what separates "the upload returned OK" from "the video plays". Still unexplained: production answered `500` where every reachable path in that handler answers `413`. This change routes around that class of failure without diagnosing it, and saying so is more useful than calling it fixed.

**pdf.js is vendored as the `pdfjs@6` pack, so an app can quote a document instead of paraphrasing it.** An app that reads a tender, an invoice or a report could previously only assert what a model remembered of it; with the page number in hand it can quote, and the reader can open that page and check. pdfjs-dist 6.1.200 (Apache-2.0), ESM build plus worker, at `/lib/pdfjs@6/`, with the directory carrying the major exactly as `chartjs@4.js` does. The `ai_doc` leads with the worker because that is where this fails: `workerSrc` must be set and must be **relative**, since a worker has to be same-origin and `/lib/` is served on every app subdomain, so the absolute `aimeat.io` form is blocked while the relative one resolves. It also states the three things a builder otherwise learns the hard way — a browser cannot fetch a third-party PDF because that origin sends no CORS headers, `getTextContent` returns positioned fragments so grouping by `transform[5]` comes before joining, and a scanned PDF yields no text items and should be reported as a scan rather than as an empty document.

**A control character in a public intake submission is stripped rather than answered with a 500.** Found by attacking the live public form on aimeat.io rather than by reading the code: a NUL byte in a field answered an **anonymous** caller with `500 INTERNAL_ERROR`, reproducibly, because Postgres cannot store U+0000 in a text or jsonb value and the write threw after every check had passed. Nothing was written and nothing leaked, so this was never a breach; it is still worth closing, because a public endpoint that answers "an unexpected error occurred" to input anyone can send cannot be told apart from a bug by the person sending it, and it hands a log-filler to anyone who wants one. C0 controls are now stripped from submitted string values, keeping tab, newline and return, which are the three a pasted form field legitimately carries. Stripping was chosen over refusing because someone pasting from a PDF should not be told their name is invalid. Everything else the same attack run tried behaved correctly and was left alone: the allow-list keeps extra fields out of the record, a submitted `id` is replaced by the server's UUID, submitted values cannot overwrite server defaults, wrong types are `422` against the locked schema, an over-long value and a missing required field are `400`, the honeypot writes nothing and says ok, and the per-IP rate limit cut the run off at 20 in 900s.

## [2.2.0] - 2026-07-26

**A file can now be handed to an agent, and the reason it could not before was a lookup rather than a permission.** Storage on an AIMEAT node is keyed by `(owner, key)`, and every agent-facing read path was namespaced to the caller. So a PDF the human owner uploaded answered `File not found` to that owner's own agent; a DM attachment arrived carrying a `storage_key` and nothing else, which is an unopenable value because the namespace is exactly what is missing from it; and `aimeat_task_create` accepted an `attachments` field with a `201` and silently dropped it, so delegating file-shaped work looked like it worked and did not. The access rules were never the obstacle: `visibility:'owner'` already admits every agent and app of the same owner, `members`/`group`/`workspace` admit theirs, and a consent grant covers the rest. Measured against a live node before a line was written — owner uploads with `visibility:'owner'`, the agent reads `GET /v1/pub/{owner}/{key}` with its own token — the bytes came back identical (sha256 over a payload containing non-UTF8 bytes), the private copy came back `403`, a foreign owner's agent came back `403`, and an unauthenticated caller got nothing. The capability existed and was undocumented; that is a worse failure than a missing feature, because it reads as "the platform cannot do this".

`services/file-refs.ts` is now the one place that turns a reference — `"<gaii>/<key>"`, the same form `ctx.files.read` already took — into either an authorized presigned handle or a stated refusal, through the same `authorizeRead()` guard `/v1/pub` uses. Bytes never pass through it. A handle for someone else's file carries a deliberately shorter TTL (15 minutes) than one for your own (an hour), so it cannot long outlive a grant the owner revokes. Built on that:

- **`aimeat_storage_download` reads by reference.** A new `owner` parameter (or an `"owner@node/key"` key) opens a file the caller does not own, and a denial now names the fix instead of stopping at "not found": store it with `visibility:'owner'`, or grant consent for `storage:{key}`. Moved with the upload tool into `src/mcp/core-storage.ts` to stay inside the 800-line rule.
- **DM attachments carry an openable `ref`** (plus `origin_ref`, `owner_ghii`, `size`, `mode`) on `aimeat_dm_inbox` / `aimeat_dm_thread`. When an attachment was duplicated into the recipient's storage, `ref` names that local copy — the one the recipient side reliably owns — and `origin_ref` still names the sender's, so the two are distinguishable rather than conflated.
- **Tasks carry file attachments.** `resources.files` rides the existing `resources` JSON column, so **neither storage backend needed a migration**. The two rules are asymmetric on purpose: on create the file must exist **and the creator must be able to read it** (which stops a task from laundering a reference to data the creator cannot see into someone's queue), with `mime` and `size` taken from the stored file rather than the request body; on read the entry is authorized again as the *reading* agent and a fresh URL is minted per read, so revoking access stops the URLs from the next read onward while the task keeps the reference.
- **`GET /v1/pub` gained `?mode=handle`**, the same access decision in a different representation, which is what lets a liaison or an MCP-only agent obtain a URL without pulling binary through a tool result.

Two pieces of debt surfaced while reading the path and were removed in the same change. The cross-owner branches on `GET`/`HEAD /v1/storage` were **unreachable**: the lookup above them is already scoped to the caller, so `file.ownerGaii !== caller` could never be true and the `authorizeRead()` there had never run once. Cross-namespace reads now have exactly one door, and that route's `404` names it. `parseRef` also existed twice; the extension sandbox now delegates to the shared implementation, so the reference form is defined once.

The Python liaison ships the crew-facing half as **`aimeat-crewai` 0.17.0**: `read_file`, `file_handle`, `upload_file`, `inbox_files`, `task_files`, `delegate_file`. It fetches bytes from the presigned URL and **never through the serve loopback**, which serialises a response as JSON/UTF-8 and therefore corrupts binary irreversibly — not an optimisation but the difference between a readable PDF and a broken one. `upload_file` defaults to `visibility='owner'` rather than `private`, because a result nobody else may read is the exact failure this work exists to remove.

Proven rather than asserted. New `test/e2e-agent-file-handoff.ts` (17) pins the whole chain and, decisively, asserts binary integrity with sha256 over non-UTF8 bytes — a UTF-8 round trip replaces those silently, which is how "the download works" hides corruption. It also pins the refusals: the private copy stays private to the same agent, a foreign owner's agent is refused, a file the creator cannot read cannot be attached (`403 FILE_ACCESS_DENIED`), a bad reference is rejected (`400 FILE_NOT_FOUND`), and flipping the file back to `private` turns a previously granted task attachment into a stated `denied` with no URL. Green on **both** supported backends together with `agent-tasks`, `storage-visibility`, `ext-files`, `agent-dm`, `attachment-sweep`, `mcp`, `mcp-consent` and `connect-serve-loopback` (210 assertions per backend); unit suite 1035, `pytest` 131 including 11 new. One bug was caught by the existing suite rather than by inspection: an em dash in a `res.setHeader` value makes Node throw `ERR_INVALID_CHAR`, which turned that route's `404` into a `500`. Header values are ASCII.

**Three surfaces stopped answering with nothing when a caller ran into the same-owner identity model, and the two that made a silent wrong answer possible are now gated.** Memory on an AIMEAT node is keyed by the **writer**: an app writes as the owner's GHII, an agent writes as its own GAII, and an owner-scope read resolves GHII first. That model is sound and deliberate, but the tools hid it completely. `aimeat_memory_read` answered a bare "Memory not found" for a key an app had plainly saved; `aimeat_memory_list` with `owner_scope` dropped every value without saying so; and `aimeat_memory_write` silently accepted a write that owner-scope reads would then shadow. Read together, the three amount to "this platform cannot share data between an app and an agent", which is false and which cost a real redesign before the credential question was asked instead. An app-grant token (`sub` = the owner GHII, `roles: ['app']`) reads and writes the owner's own records exactly like the app does; the question was never whether an agent *may*, only which credential it runs as.

So each surface now names the thing it used to withhold. `memory_read` looks once across the owner scope **on the miss path only**, where the answer was already an error, and returns `NOT_IN_YOUR_NAMESPACE` carrying the identity that actually holds the key, the `GET /v1/memory/{key}?owner_scope=true` call that returns its value, and the app-grant route for writing it. `memory_list` reports `values_omitted` plus how to read one value. `memory_write` returns `SHADOWED_BY_OWNER_COPY` when the owner already holds that key, because the write succeeds into the caller's own namespace and then never surfaces anywhere. That last case was the sharpest edge: `dedupGhiiFirst` did not merely rank the loser lower, it **removed** it, so an agent's newer copy vanished even from an owner-scope listing with nothing to indicate a collision had happened. The survivor now carries `alsoUnder` (`also_under` over REST) naming every other identity holding the same key. While fixing it, `services/owner-memory.ts` turned out to hold a **second** copy of the same ranking loop, despite the repository's own comment claiming it had been moved there "so every owner-scope read shares one implementation"; it now delegates, so the rule exists once.

**A presigned upload silently ignored the options the tool had just accepted.** `aimeat_extension_install({ update: true })` destructured the flag and then minted its token with `meta: {}`, so the flag never reached the server and the upload still failed `ALREADY_EXISTS`, forcing a delete-and-reinstall that also discards the extension's `ext:{name}` memory. The same shape of bug is already on record for app publishing, which is the tell: the meta was hand-written at every minting site. `PRESIGNED_META_KEYS` + `buildUploadMeta()` now declare the carry-over list once, next to the token. The extension upload path honours `update` (a real upsert over the same code-and-metadata field set as `PUT /v1/extensions/:name`, lifecycle preserved, `403` on an owner mismatch) and `activate`. `POST /v1/extensions` also gained `mode: 'presigned'`, mirroring `POST /v1/apps`, which means the mint and the PUT can happen in a single command and the upload token never has to be transcribed by hand.

**The sandbox lacked the two primitives every extension author was forced to invent.** QuickJS has no `crypto.subtle`, so a browser app hashed its values with SHA-256, its extension hashed the same values with something else, and every value the app computed looked stale to the server: it would have recomputed and re-charged the same AI work indefinitely. `ctx.hash(s)` and `ctx.now()` are now available as pure guest-side helpers with no host call and no API-budget cost, and the exact source of `ctx.hash` is exported as `EXT_HASH_REFERENCE_JS` so a browser app can compute the identical value rather than approximate it. `ctx.now()` is captured once per action, so a script that stamps several records stays internally consistent. Reaching past them now warns at install time, on the inline path and the ZIP path alike: `crypto.subtle`, Node `crypto`, `Date.now`, `Math.random` and `eval` each produce a named, non-blocking note explaining what the sandbox actually offers.

**`openapi.yaml` had quietly stopped being machine-readable in two ways, and Rule 3 had no gate behind it.** One `$ref` pointed at `#/components/schemas/SuccessEnvelope`, a schema that does not exist (the canonical one is `AimeatEnvelope`, used by every other response in the file), and that single dangling pointer broke `pnpm generate:types` outright: Rule 3's own tool could not run. Separately, `description: Whole document portion public? }` is a plain scalar inside a flow mapping. Worth being exact about what that did and did not break, because the first diagnosis overstated it: the repo's `yaml` package accepts it in every mode (plain, `strict`, `version: '1.1'`, and both together) and so does openapi-typescript, so nothing here ever complained. PyYAML rejects it, and PyYAML is what most Python OpenAPI tooling is built on, so an outside consumer would have seen a syntax error instead of the API and we would never have heard about it. Both are one-line fixes; the durable part is `scripts/check-openapi.ts`, which parses the spec, resolves all 2642 internal `$ref`s and flags plain flow scalars carrying YAML indicators, wired into the pre-commit hook and CI. Its portability check walks the AST rather than the text, because a line-based scan cannot tell a real flow mapping from a `{ ... }` that is merely prose inside a block description, and this file is full of the latter: the first attempt produced 12 false positives against 1 true one.

Proven rather than asserted. This behaviour had **no** test coverage at all, which is why it could drift silently: `grep owner_scope` across `test/` returned five hits, every one of them in an organism suite. Two new suites now pin it. `e2e-memory-namespaces` (10) drives all four principals and asserts that an agent token writes its own GAII, that an app-grant token writes the owner's GHII, that `?owner_scope=true` is what crosses the boundary, and that a shadowed copy is reported in `also_under` instead of disappearing. `e2e-presigned-meta` (12) asserts the upload round trip end to end and, decisively, that the sandbox's `ctx.hash` equals the published reference implementation byte for byte, which is the single assertion that would have caught the drift that started all this. Both green on **both** supported backends. Full sweeps: 3741 assertions on SQLite and on PostgreSQL+Kysely, with the pre-existing failures confirmed by re-running the same suites on a pre-change tree and observing the identical result (they are environment-dependent: local OAuth provider config, an absent money-rail handler, SMTP). `generate:types` runs clean for the first time in this tree, and the openapi guard was verified by re-injecting each break in turn and watching it report the dangling ref with its JSON path and the unquoted scalar with its line.

**A euro price now settles on-chain in EURC, on the same non-custodial x402 rail that already settled dollars in USDC (TARGET-042).** Since 2.0.0 a USD-priced resource could be paid in USDC; a EUR price had no on-chain path at all, and the reason was one hardcoded line — the handler advertised `currencies: ['USD']` and the network registry held a single asset per chain. The registry is now **currency-aware**: each network carries one settlement asset per money currency (USD → USDC, EUR → EURC), and every surface derives what it offers from that map instead of naming a token. The rail itself is unchanged — same `exact` scheme, same facilitator, same non-custodial settlement straight to the seller's own address — and so is **model 2**: the session currency stays `EUR`, the stablecoin is only the instrument. One EVM address receives both tokens, so a second currency asked sellers for nothing new.

The governing rule is **advertise only what can settle**. A `(network, currency)` pair with no asset is *absent* from the map rather than an empty entry, and that absence is the gate: the handler's currency list, the 402's `exact` scheme and the payout endpoint all resolve through `getX402Asset()` first, so a currency reaches a buyer only when a real asset backs it. An unknown network yields `[]`, meaning a misconfigured node promises nothing rather than a rail it cannot honour. `/.well-known/ucp` needed **no change** — it reads the handler's list, so EUR propagated on its own.

Every value was verified against the live chain, not written from memory. Addresses come from Circle's own documentation; `symbol`, `decimals` and the EIP-712 domain were then read from the deployed contracts and **proven** by recomputing each contract's own `DOMAIN_SEPARATOR` from `(name, version, chainId, address)`. All four match, and the same method reproduced both pre-existing USDC entries — including the quirk that Base mainnet USDC's domain name is `USD Coin` while every other entry equals its ticker. EURC is `0x8084…359F` on Base Sepolia and `0x60a3…db42` on Base mainnet, both `name: EURC`, `version: 2`, 6 decimals.

That domain was the real risk: a wrong `name` invalidates every buyer signature and would surface as `invalid_exact_evm_signature` — the same error the suite's two fixture tests already produce against a live facilitator, so a mistake would have hidden behind a familiar failure. It was therefore settled empirically. Probed with a real EIP-3009 signature from a throwaway key, the public `x402.org` facilitator answered EURC with `invalid_exact_evm_insufficient_balance`, **identical to the USDC control** — the asset was accepted and the signature checked against EURC's own domain. A deliberately wrong name (`Euro Coin`) came back `invalid_exact_evm_token_name_mismatch`, a distinct error, so a wrong value fails loudly. What that facilitator does *not* do is serve Base mainnet: `/supported` lists networks, not assets, and `eip155:8453` is absent — equally true of USDC today, not a limit EURC introduces. Mainnet needs a facilitator that serves it and remains a separate, deliberate decision.

Both tokens carry 6 decimals, so micros map to atomic units 1:1 — but the money chokepoint gained a genuine decimals-aware `microsToTokenRaw` rather than a rename, returning a string so an 18-decimal token cannot silently overflow, and ceiling when scaling down so a seller is never handed less than the price. `GET /v1/commerce/payout` now reports `currencies` plus `assets` (`{currency, symbol, address, decimals}`) in place of a hardcoded `currency: 'USDC'`, and Profile → Wallet reads `EUR → EURC · USD → USDC` (EN + FI together). `openapi.yaml` updated in the same commit for the changed response shape.

Proven rather than asserted: `e2e-x402` goes 11 → 16 — a EUR offering's 402 carries the EURC asset and its domain, a EUR session settles with its tracking code naming EURC, and the test that protects buyers asserts that **every advertised currency really produces a settleable `exact` scheme**, so nothing is offered that cannot be paid. The negative case (a network lacking a currency's asset must not advertise it) lives in a new `x402-registry.test.ts` against a synthetic USD-only network, since no real network has a missing pair any more; that file also pins each asset's address, decimals and domain so a silent edit cannot quietly invalidate signatures. 16/16 with `e2e-commerce` 34/34 on **both** supported backends (SQLite and PostgreSQL+Kysely), unit suite 1033/1033, and the Wallet tab driven in a real browser. The suite's two fixture tests fail identically before and after when run without the test facilitator (they need a live one) — baseline confirmed on a clean tree first.

**Live updates announce themselves the moment a stream opens, and a sandboxed app is no longer told about changes it has no scope to see.** The SSE live-update stream (`GET /v1/events`) sent nothing until its 30 second keepalive, so a freshly opened connection was indistinguishable from a hung one: the client could not tell it was connected, an intermediary with a short read timeout could drop the stream before a single byte arrived, and anyone debugging an app reasonably concluded that SSE itself was broken. Measured against production before the fix: first byte at 30,077 ms, content `:keepalive`, identical on the apex and on an app subdomain (so the app origin was never the problem; the silence was). The stream now flushes `retry: 3000` plus an `:open` comment on connect, and keeps alive every 15 seconds instead of 30, which also holds it under the read timeout of common proxies. Both are comment lines, so no new information is exposed.

Reading that path surfaced two authorization holes, fixed in the same change:

- **Change domains are now scope-gated for restricted principals.** An app-grant token resolves to the owner's GHII, so an app holding only `memory:read` was told the name and the timing of every change across the owner's entire keyspace, including `messages`, `wallet` and `agent-tasks`. The stream carries no payload beyond domain names, but a name plus its timing is metadata the owner never consented to hand that app, and fencing exactly this is what H-2 app-origin isolation exists for. A new `src/auth/sse-domain-scopes.ts` maps each domain to the scope that reveals it (`memory` behind `memory:read`, `files` behind `storage:read`, `organisms` behind `organism:read`, `agent-tasks` and `agents` behind `task:read`, `messages` behind `messages:read`, and so on), the decision is frozen at ticket-mint time where `req.auth` still exists (the stream itself only carries the ticket), and a domain absent from the map is **denied**, so a domain added elsewhere in the node cannot silently start leaking to sandboxed apps. Owner sessions on the apex portal stay unfiltered and unchanged. Note for app authors: an app that subscribes to a domain outside its granted scopes stops receiving it, and the fix is to request the matching scope.
- **Only a real owner session may mark the owner present.** Every app-grant stream used to flip presence, so an app left open in a tab pinned the human "available" to everyone entitled to see them. Presence eligibility is now decided with the ticket, and an app stream neither marks the owner online on connect nor offline on disconnect.

Evidence rather than assertion: `test/unit/sse-domain-scopes.test.ts` (14 tests) covers the gate itself, including that no combination of app scopes reveals an operator domain, and `test/e2e-sse.ts` (7 tests, self-spawning with the app origin enabled) proves the wiring with a real app-grant token. Its decisive pair: an organism change reaches the owner stream and never the app stream, while the same app stream still receives the `memory` domain it *is* scoped for, which shows the gate is selective rather than the stream simply being dead. Green on both supported backends (SQLite and PostgreSQL+Kysely); `e2e-presence` (16) and `e2e-app-origin` (7) unaffected.

## [2.1.0] - 2026-07-21

**Any MCP client can now act on the EXCHANGE marketplace — not just a bespoke fleet runtime. Call another app's tool through a contract, run agent-work, and renegotiate a contract, all over MCP.** The two-sided marketplace's *browse / accept / bid* surface was already on MCP, but three operations had no MCP tool — so a plain MCP-connected agent (a Claude chat, any framework) could not do what a hand-written runtime could. They exist now, on **both** MCP surfaces, each a thin wrapper over the SAME routes + settlement the REST paths use (no new business logic, pricing stays authoritative from the provider, the node stays the trust boundary):

- **`aimeat_app_tool_invoke`** — call an app's offered tool (a method like `getCompanyBrief`) through *your* metered contract. The generic "one app/agent calls another app's function" channel: the accepted contract authorises it, the node meters + rakes + routes to the pinned interface version, and the provider's own upstream API keys stay server-side (you never see or need them). A throw inside the tool refunds you.
- **`aimeat_exchange_work` / `_work_deliver` / `_work_list`** — the async agent-work surface: start a task under a contract, deliver it as the provider (settle-on-delivery), and list your items on either side.
- **`aimeat_exchange_proposals` / `_proposal_decide`** — renegotiation: list the proposals you're party to, and accept (supersede at the agreed terms) / decline / withdraw. Mutual consent is the authority — a proposed price only binds once the other party accepts.

Both surfaces are covered so **every** agent gets them: the **public `/v1/mcp`** (hosted clients like a Claude chat — the server tool threads the caller's session token so the app-tool invoke can run the backing capability as them), and the **connector MCP** (`aimeat connect serve` — tunnelled fleet agents get them as thin REST proxies, no bespoke code needed). Settlement is Express-`res`-coupled in the gateway, so the two settling tools drive it through a capture-only mock `Response` and translate the outcome to an MCP result. Scope-gated `exchange:read` / `exchange:write` like the rest of the family. New E2E `test/e2e-exchange-mcp.ts` (10/10 on SQLite + Postgres+Kysely: happy paths + failure modes incl. on-delivery settlement, renegotiation supersede, and refuse-before-charge); the existing `e2e-exchange` stays 59/59 on both backends. The connector tools ship in the `aimeat` npm package (`dist/`), so a published-npm fleet picks them up on the next release; a repo-linked `aimeat connect serve` gets them from a rebuild.

**Email becomes a first-class account identifier: connect an agent by email or handle, one verified email belongs to exactly one account per node (DB-enforced), and — on gated nodes — an account can't be created without one.** A three-part identity change, all node-side (the connector CLI is unchanged — it forwards `--owner` verbatim, so every already-installed `aimeat connect` starts accepting an email the moment the node is updated; no CLI republish).

- **Connect by email *or* handle.** `POST /v1/agents/device-authorize` (the connector's `aimeat connect --owner …`) accepts `owner` as **either** the account handle (as before) **or** the account's **verified email** (case-insensitive): a value containing `@` is resolved to the owning account's handle before the RFC 8628 flow, so a new user who knows their email but not their handle can still connect. The email only *selects* which account the device request targets — it is never an auth factor (the human still approves the device code while signed in), so the flow's security is unchanged. No verified account for that email → `404 NO_ACCOUNT`; malformed → `400 INVALID_EMAIL`; (invariant-protected) more than one match → `409 AMBIGUOUS`.

- **One verified email ↔ one account, per node — enforced at the database.** A partial-unique index on `emailHash` (both backends, dedupe-safe migration) makes a duplicate impossible. `emailHash` became a strictly *verified*-email binding: `register-web` no longer stamps an unverified email at account creation (deferred to `verify-email`), and every verify/provision path refuses an email already verified elsewhere (`409 EMAIL_TAKEN`) ahead of the index. The email→owner resolver reads the full match set and asserts the invariant (logs + `409 AMBIGUOUS`) rather than silently picking one.

- **Registration requires an email when the email gate is on.** With `AIMEAT_EMAIL_CONFIRMATION_REQUIRED` set, an account can no longer be created without an email (`400 EMAIL_REQUIRED`) on either `POST /v1/ghii` (which now accepts an optional `email`, records it, and sends a verification code, returning a `verification_id`) or `register-web`. Social sign-in (Google / Casdoor / tenant-pinned Microsoft Entra) satisfies the gate automatically — the IdP's email is verified at that instant, no confirmation cycle. Nodes with the gate **off** (the default) are unchanged: accounts may exist with just a handle and no email, and connect-by-email simply resolves only those accounts that actually verified an address.
...
**It is verified against real Stripe test mode, not asserted.** The full acceptance ran end to end — a KYB-verified seller sells a EUR offer, the buyer is charged with test card `4242` on a real PaymentIntent, the 0.75 EUR application fee lands with the operator, the charge settles on the seller's Express account, and DAC7 + VAT are booked — **17/17**, cross-checked against the Stripe API. The four new routes are live in production. Two hard limits held throughout: **test mode only** (`sk_test_`) and **non-custodial** (direct charges never route through the operator balance). Live activation of the operator's Stripe account (Y-tunnus + bank + ToS) remains the operator's own step.
