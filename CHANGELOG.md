...
**Sharing groups were a management API attached to nothing, and the tests said so if you read them.** `e2e-sharing-groups.ts` had 17 assertions covering create, list, add member, change permissions, remove and delete, and not one that read or wrote a memory record; the strings `group_id` and `visibility` did not appear in the file. `e2e-mcp-consent.ts` had 8 and never called `/v1/memory`. So both features had a fully tested management surface and no test that they affected access at all, which is the class `e2e-mcp-cross-owner.ts` had named in its own header two days earlier. A sibling project measured cross-owner reads on production, got 403 everywhere, and concluded the platform could not do it. Measured here on both backends before anything was touched, four defects, each of which answered `200` while doing nothing:

- **SQLite never wrote the `groupId` column on any memory path.** Not the INSERT, not either UPDATE, not the compare-and-swap. Every `visibility:'group'` record on a SQLite node was unreadable by every member of the group it named, permanently, with the group's own name echoed back in the write response.
- **Neither backend wrote it on an UPDATE.** Postgres set it on first INSERT only, so a record created `private` could never become group-visible afterwards. That is the ordinary sequence — write the thing, then decide to share it — and it is almost certainly what the field report hit.
- **Moving a record between groups did nothing.** Same root cause, worse consequence: the old audience kept reading and the new one never gained access, while the write reported success and the UI showed the new group's name.
- **The cross-owner read door passed the raw JWT `sub`.** An owner session carries a bare account name, not the GHII, so no sharing-group membership and no consent grant could ever match it — a person could not read what had been shared with them while their own agents could, which inverts the guess in the field report. The storage-file twin `GET /v1/pub` had fixed exactly this in July with a comment explaining why; this side of the pair was left.

Diagnosis proven rather than argued: the two candidate fixes were applied locally and the whole measurement re-run on both backends before anything was written for keeping. `storage/memory-sharing.ts` now holds the one rule about which group a record belongs to after a write, called by both providers, and it inherits — a writer that says `group` without naming one keeps the group the record already had, the same reasoning `trackable` uses a few lines away, so an agent appending today's output to a shared key cannot silently end the share by forgetting to resend an id.

**Then the record-field shape itself went, because it could not express what sharing is for.** `visibility:'group'` plus one `groupId` can name exactly one group, has to be repeated on every write, and cannot cover a key space that does not exist yet. The last is fatal for the only use anyone has: a subscription writes tomorrow's record tomorrow, and the reader must not need a second act from anybody to see it. A share is now its own row — `(owner, group, key pattern)` in `group_shares` / `GroupShare`, indexed both ways because the group direction is on every cross-owner read and the owner direction is the question ownership is judged by. The record keeps its own visibility, normally `private`: visibility is the floor and a share is a named exception on top, so sharing something never changes what it is, and one record can sit in several shares. The wildcards are the consent ones on purpose (`*` one segment, `**` the subtree) — a node where one wildcard means two things depending on which feature reads it is a node where nobody can predict what they agreed to.

Both list directions exist, and the reader's is the half whose absence made the feature unusable in practice: without `GET /v1/shares/incoming` you had to be handed an owner identity and an exact key out of band, because nothing on the node would tell you what you had been given. Making a share is its own permission, `share:manage`, which no wildcard carries. Two things fall out of that and both are recorded because they generalise: the gate **cannot** be a role check, since an agent's token carries its owner's `owner` role (`routes/auth.ts` copies it on) and `roles.includes('owner')` therefore passes every agent of every owner — the same pattern still sits in `services/consent-write.ts`, where it means an agent can grant consent without `consent:manage`, and that is left as a separate finding rather than fixed in passing. And a pattern with no literal prefix, or one reaching a reserved server key (`openrouter.*`, `ai-usage.*`, `profile.*`), is refused rather than trimmed, because a share that quietly covers more than the person believed is worse than an error.

Existing `visibility:'group'` records keep working; nothing is broken while they have not moved. `scripts/migrate-group-shares.ts` converts them, one share per record at that record's **exact** key, and is deliberately not a boot step: it creates access rights, and access rights should not appear because a process restarted. Dry run by default, idempotent, and it never widens anything — turning a per-key share into a `space.**` pattern is an owner's decision, not something a migration should guess. Run it once the production numbers are known.

The surfaces followed the model rather than the other way round. The Access tab lists what each group reaches with add and revoke and a count on the collapsed header, plus **Shared with you**; the Memory tab shows a `shared · N` badge on a row a share covers (the visibility pill still reads `private`, because it is) and can share the key's own space pre-filled one level up, and `group` left every visibility picker: the popover, the detail select, the bulk bar, the create form and the edit modal. One defect there was found by the browser and not by reasoning — the shares list hung off `loadGroups`, which a seeded page never calls, so every group rendered as sharing nothing, which is worse than rendering nothing because it reads as an answer.

Proven rather than asserted. `e2e-key-space-shares` (29) and `e2e-group-sharing` (20) green on **both** supported backends, alongside sharing-groups, mcp-consent, memory-namespaces, memory-full, storage-visibility, mcp-memory-extended, workspace-member-records, workspace-public-records, organism-workspace-access, memory-file-presigned, mcp-cross-owner and app-grants. The app-grant path is measured rather than assumed, because the subscription flow's shape depends on it: an app-grant token **with** `share:manage` creates a share and **without** it gets 403, and it may not create a group or admit a member — an app can hand out access to a key space but cannot assemble its own audience. The migration was run end to end on a seeded legacy record: dry run, apply, re-run reporting it already covered, then the reader finding it in `/v1/shares/incoming` and reading the key. The UI was driven in a real browser at 390x844, 1280x900 and 1280x460 (horizontal overflow 0 at each), with 0 repaints on an open panel over 20 idle seconds and one request (`/v1/build`) over 60.

One loose end worth naming: `share:manage` existed on the node and in the app-grant vocabulary but had no row in the agent permissions editor, so an owner could not tick it for an agent at all. Found while merging, by reading a rule another branch had just added. The round-trip test compares the frontend list against the frontend domains, so a scope missing from both sides passes — the test has that blind spot whatever this change did.
