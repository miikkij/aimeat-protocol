...
/* ── Mindmap builders (radial) — labels resolve clicks by text; heat via :::heatN + CSS ── */

function heatClass(opts, n) { if (!opts.heatmap) return ''; const b = heatBucket(n); return b ? `:::heat${b}` : ''; }
...
function buildOrgMindmap(graph, opts) {
  ...
  (graph.workspaces || []).forEach((w) => {
    ...
    lines.push(`    ${wsLabel}${heatClass(opts, wsTotal)}`);
    if (opts.level === 'spaces' || opts.level === 'counts') {
      (w.spaces || []).forEach((sp) => {
        const c = opts.level === 'counts' ? ` · ${sp.count || 0}` : '';
        lines.push(`      ${mmLabel(sp.name)}${c}${heatClass(opts, sp.count || 0)}`);
      });
    }
  });
  if (opts.showUsers) {
